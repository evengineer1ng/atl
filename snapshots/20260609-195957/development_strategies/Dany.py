...
class Dany(IStrategy):
    ...
    def custom_exit(self, pair: str, trade, current_time: datetime, current_rate: float,
        ...
        is_expanded = bool(last_candle.get('expansion_event', False))

        # Fade: early underwater exit if expansion dies immediately
        ...
        # Time: maximum observation window to keep telemetry clean
        if duration_minutes >= 20:
            return 'ignition_time'

        # Early time-out: expansion died and we're flat after 8 minutes
        if duration_minutes >= 8 and duration_minutes < 20 and abs(current_profit) < 0.003 and not is_expanded:
            return 'ignition_time'
