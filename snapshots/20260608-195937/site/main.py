...
def build_canonical_universes() -> dict[str, Any]:
    rows = fetch_coingecko_markets(250)
    ...
    def store(name: str, bases: list[str]) -> None:
        payload = {"name": name, "bases": bases, "count": len(bases), "built_at": iso_now(), "source": "coingecko"}
        replace_generated_json(f"pairlist_universe:{name}", payload)
        universes[name] = payload

    store("top100_marketcap", [e["base"] for e in by_rank[:100]])
    store("top50_marketcap", [e["base"] for e in by_rank[:50]])
    store("top20_marketcap", [e["base"] for e in by_rank[:20]])
    by_volume = sorted(entries, key=lambda e: e["volume"], reverse=True)
    store("top50_volume", [e["base"] for e in by_volume[:50]])
    by_momentum = sorted(by_rank[:100], key=lambda e: e["chg"], reverse=True)
    store("custom_momentum_30", [e["base"] for e in by_momentum[:30]])
    # Cartographer universe (clusters the market, surfaces constellation laggards).
    # Stores its own richer payload; we add it to the return map for the cycle log.
    ...
    for record in records:
        ...
        conn.execute(
            ...
            INSERT INTO dev_archived_trades (
                ...
                trade_id, pair, exit_reason, is_short, forced,
                profit_ratio, profit_abs, realized_profit, open_date, close_date, created_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(episode_key, trade_id) DO UPDATE SET
                pair=excluded.pair, exit_reason=excluded.exit_reason, is_short=excluded.is_short,
...
def development_candidate_detail_page(request: Request, candidate_id: int) -> HTMLResponse:
    ...
    timeline = dev_strategy_timeline(str(candidate.get("slug") or ""))
    return templates.TemplateResponse(
        request,
