...
class BlockParty(IStrategy):
    '''
    Block Party (rev.7): cartographer laggard-catch-up strategy.
    Revision focus: C-grade shift fix (0 trades/6h). Entry genome deepened
    and loosened via expanded confirmation scorecard (8 signals), five
    distinct entry tracks, and lowered thresholds. Exit and Management
    genomes preserved with minor robustness tuning.
    '''
    ...
    # --- Tunable parameters (rev.7 loosened for opportunity capture) ---------
    lookback = 96               # 24h on 15m bars
    lookback_fast = 16          # 4h on 15m bars
    min_group_pct = 0.10        # allow smaller cores
    group_move_min = 0.15       # catch smaller lit moves
    group_move_fast_min = 0.10  # lowered fast-window lit threshold
    group_move_strong = 1.5     # lowered strong-move bar
    laggard_gap = 0.15          # lowered core gap
    soft_laggard_gap = 0.0      # any underperformer qualifies in strong move
    deep_laggard_gap = 0.50     # lowered deep-value track
    min_breadth = 0.10          # relaxed coherence floor
    group_exit_level = 0.15     # tighter cool-off
    rsi_overbought = 82
    rsi_oversold = 18
    vol_mult = 0.05             # very loose volume floor
    early_catchup_pct = 0.05    # easier ignition
    catchup_exit = 0.15         # tighter profit-take
    custom_time_stop_min = 90
    custom_time_stop_dd = -0.010
    bb_period = 20
    bb_std = 2.0
    macd_fast = 12
    macd_slow = 26
    ...
    # -------------------------------------------------------------------------
    # --- GROUP HELPER (used by Entry and Exit genomes) -----------------------
    # -------------------------------------------------------------------------
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Trend & momentum
        dataframe['ema_f'] = ta.EMA(dataframe, timeperiod=8)
        ...
        # Confirmation signals (rev.7: expanded scorecard, easier to satisfy)
        ...
        dataframe['sig_ema_fast_long'] = dataframe['close'] > dataframe['ema_f']
        dataframe['sig_ema_fast_short'] = dataframe['close'] < dataframe['ema_f']
        ...
        dataframe['sig_adx_long'] = dataframe['adx'] > 15
        dataframe['sig_adx_short'] = dataframe['adx'] > 15

        dataframe['sig_vol_pulse_long'] = dataframe['volume'] > (dataframe['vol_ma'] * 0.3)
        dataframe['sig_vol_pulse_short'] = dataframe['sig_vol_pulse_long']

        dataframe['sig_mom_tick_long'] = dataframe['ret_current'] > 0
        dataframe['sig_mom_tick_short'] = dataframe['ret_current'] < 0
        ...
        # Composite scores (8 components, core threshold >=2, scout >=1)
        dataframe['score_long'] = (
            dataframe['sig_catchup_long'].fillna(False).astype(int) +
            dataframe['sig_ema_long'].fillna(False).astype(int) +
            dataframe['sig_ema_fast_long'].fillna(False).astype(int) +
            dataframe['sig_macd_long'].fillna(False).astype(int) +
            dataframe['sig_rsi_long'].fillna(False).astype(int) +
            dataframe['sig_adx_long'].fillna(False).astype(int) +
            dataframe['sig_vol_pulse_long'].fillna(False).astype(int) +
            dataframe['sig_mom_tick_long'].fillna(False).astype(int)
        )

        dataframe['score_short'] = (
            dataframe['sig_catchup_short'].fillna(False).astype(int) +
            dataframe['sig_ema_short'].fillna(False).astype(int) +
            dataframe['sig_ema_fast_short'].fillna(False).astype(int) +
            dataframe['sig_macd_short'].fillna(False).astype(int) +
            dataframe['sig_rsi_short'].fillna(False).astype(int) +
            dataframe['sig_adx_short'].fillna(False).astype(int) +
            dataframe['sig_vol_pulse_short'].fillna(False).astype(int) +
            dataframe['sig_mom_tick_short'].fillna(False).astype(int)
        )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if lit_up:
            ...
            # Track 3: Ignition snap (turning laggard with fast EMA + momentum tick)
            cond_ignition = (
                is_laggard &
                dataframe['vol_ok'] &
                dataframe['sig_ema_fast_long'] &
                dataframe['sig_mom_tick_long'] &
                dataframe['sig_not_extreme_long']
            )
            ...
            # Track 5: Warm core (modest group move, decent breadth, laggard)
            cond_warm = (
                (group >= self.group_move_min) &
                (breadth >= 0.25) &
                is_laggard &
                dataframe['vol_ok'] &
                dataframe['sig_not_extreme_long'] &
            ...
            enter = cond_core | cond_deep | cond_ignition | cond_scout | cond_warm
        ...
        if lit_down:
            ...
            cond_ignition = (
                is_laggard &
                dataframe['vol_ok'] &
                dataframe['sig_ema_fast_short'] &
                dataframe['sig_mom_tick_short'] &
                dataframe['sig_not_extreme_short']
            )
    ...
    def custom_exit(self, pair: str, trade, current_time, current_rate,
        ...
        - Time stop: cut if the laggard has not moved after N minutes.
