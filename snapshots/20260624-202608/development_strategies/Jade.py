...
class Jade(IStrategy):
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Regime filter for directional bias: shorts re-qualified to confirmed bearish leadership only
        dataframe['regime_bearish'] = (
            dataframe['strong_trend'] &
            (dataframe['adx'] > 35) &
            (dataframe['lead_lag_score'] < -0.15) &
            (dataframe['basket_breadth'] < 0.4) &
            (dataframe['divergence'] < -0.001)
        )
        dataframe['regime_long_only'] = ~dataframe['regime_bearish']
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        volume_strong = dataframe['volume'] > (dataframe['volume_mean'] * 1.2)
        adx_gate = dataframe['adx'] > 24
        breakout_high_ok = dataframe['breakout_high'] & (dataframe['close'] > dataframe['high_20'].shift(1) + 0.15 * dataframe['atr'])
        breakout_low_ok = dataframe['breakout_low'] & (dataframe['close'] < dataframe['low_20'].shift(1) - 0.15 * dataframe['atr'])

        base_long = (
            breakout_high_ok & adx_gate & volume_strong &
            (dataframe['rsi'] < 80) & (dataframe['rsi'] > 30)
        )
        base_short = (
        ...
        h4 = base_short & (dataframe['divergence'] < 0) & bearish_ok
        dataframe.loc[h4, 'enter_short'] = 1
        dataframe.loc[h4, 'enter_tag'] = 'h4_breakdown_divergence_short'

        h5 = base_short & (dataframe['divergence_z'] > 2.0) & (dataframe['rsi'] < 65) & bearish_ok
        ...
        h6 = base_short & (dataframe['vol_ratio'] > 1.5) & dataframe['strong_trend'] & (dataframe['rsi'] > 50) & bearish_ok
        ...
        if btc:
            ...
            h11 = base_short & (dataframe['basket_vol'] > dataframe['volatility'] * 1.3) & (dataframe['divergence'] < 0) & bearish_ok
            ...
            h14 = base_short & (dataframe['basket_breadth'] < 0.4) & (dataframe['basket_breadth'] < dataframe['basket_breadth'].shift(3)) & bearish_ok
            ...
            hA_short = base_short & (dataframe['divergence_z'] > 1.0) & bearish_ok
    ...
    def custom_exit(self, pair: str, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        # Gated time-stop: raised loss threshold + structural confirmation required
        # Only fires if trade is at/near its worst observed price (no intrabounce recovery)
        if current_profit < -0.01 and duration > timedelta(minutes=45):
            if current_rate <= trade.min_rate * 1.001:
                return 'time_stop_loss_confirmed'

        # Flat exit: much smaller profit, longer hold required
        if 0 < current_profit < 0.003 and duration > timedelta(minutes=90):
    ...
    def adjust_trade_position(self, trade: 'Trade', current_time: 'datetime', current_rate: float,
        ...
        # Only add to proven winners; never scale into losers
        ...
        if (current_time - trade.open_date_utc) < timedelta(minutes=20):
            return None
        # Research-scaled add: 30% of original stake
