...
def resolve_backtest_strategy(strategy_key: str) -> dict[str, Any]:
    """Resolve a strategy_key (official team id / display name / strategy family) to
    {module, klass, timeframe, strategy_path} for backtesting."""
    ...
    if not strategy_path or not strategy_path.exists():
        raise ValueError(f"Strategy file not found for '{strategy_key}'.")
    klass = strategy_class_in_file(strategy_path) or strategy_path.stem
    return {
        "module": strategy_path.stem,
        "klass": klass,
        "timeframe": str(instance.get("timeframe") or "5m"),
        "strategy_path": str(strategy_path),
        "team_id": instance["id"],
        "display_name": instance.get("display_name", strategy_key),
    }


def make_universe_backtest_strategy(strategy_module: str, strategy_class: str) -> str:
...
def run_universe_backtest(
    ...
    wrapper_class = make_universe_backtest_strategy(strat["module"], strat["klass"])
