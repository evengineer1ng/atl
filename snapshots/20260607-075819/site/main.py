...
ML_TELEMETRY_CATEGORIES: list[dict[str, Any]] = [
    ...
    {"name": "champion_exit_rate", "good": "high", "measurable": True},
...
def preview_official_team_season_review(team: dict[str, Any], season: dict[str, Any]) -> dict[str, Any]:
    ...
    efficiency_score = clamp(max(avg_roi, 0.0) * 3.0 + min(champion_exits, 6) * 1.5 + min(len(closed_trades), 10) * 0.3, 0.0, 15.0)
    rubric = [
        ...
        {"label": "Efficiency", "score": round(efficiency_score, 1), "max_score": 15, "note": f"{avg_roi:.2f}% average ROI and {champion_exits} champion exits."},
    ]
...
# ===========================================================================
ML_CHAMP_EXITS = {"champ_dynamic_roi", "champ_dynamic_roi_hit"}
...
def ml_collect_trades(strategy: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if kind == "main":
        ...
        return [dict(row) for row in rows]
    # dev kind: source_team_id holds the dev_candidates slug.
    db_path = resolve_path(strategy.get("source_db_path")) or _ml_dev_db_path(str(strategy.get("source_team_id") or strategy.get("slug")))
    if not db_path:
        return []
    return _ml_read_sqlite_trades(db_path)
...
def ml_compute_telemetry(strategy: dict[str, Any], trades: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    wins = len(winners)
    champ = sum(1 for t in closed if str(t.get("exit_reason") or "") in ML_CHAMP_EXITS)
    ...
    measured = {
        ...
        "champion_exit_rate": percentage(champ, sample) * 100.0,
...
def ml_create_descendant(hypothesis: dict[str, Any]) -> dict[str, Any]:
    ...
    hypothesis_text = (
        f"ML Evolution Review descendant of {name_a} × {name_b} "
        f"(conviction {hypothesis['conviction_score']:.2f}). "
        f"Inherit from {name_a}: {', '.join(strengths_a) or 'core behavior'}. "
        f"Inherit from {name_b}: {', '.join(strengths_b) or 'core behavior'}. "
        f"Target to cover weaknesses: {', '.join(weaknesses) or 'none catalogued'}."
    )
    expected_behavior = (
        f"Combine the complementary strengths of {name_a} and {name_b} so each parent's strength "
        f"covers the other's weakness. {hypothesis['rationale']}"
    )
    ...
    payload = {
        ...
        "strategy_notes": expected_behavior,
        "long_short_mode": "both",
        "expected_behavior": expected_behavior,
        "risk_profile": "Hybrid prospect — inherits both parents' mechanisms; monitor for over-trading.",
...
def run_ml_evolution_review() -> None:
    ...
    if get_setting("ollama_api_key", ""):
        try:
            user_payload = {
                ...
                "note": "These results are precomputed and authoritative. Do not invent numbers or propose additional descendants.",
            }
            content = ollama_chat(
                [
                    {
                        "role": "system",
                        "content": (
                            "You are the ATL strategy biology committee writing a 15-day Evolution Review. "
                            "You are given precomputed aggregates and the descendants that already passed hard thresholds. "
                            "Do NOT invent numbers or add descendants. Return strict JSON {report_markdown, summary}. "
                            "report_markdown: a readable narrative review of the cycle window — what recurred, which "
                            "strategies stayed divergent, which pairings matured, and why the proposed descendants "
                            "(or the absence of any) make sense. summary: one-sentence headline."
                        ),
                    },
                    {"role": "user", "content": json.dumps(user_payload)},
                ]
    ...
    report_json = {"aggregates": aggregates, "proposed_descendants": created, "closest_misses": rejection_reasons}
    with closing(get_db()) as conn:
        conn.execute(
            """
...
def seasons_page(request: Request) -> HTMLResponse:
    return templates.TemplateResponse(
        request,
        "seasons.html",
        page_context_bundle("seasons", "Season Office", page_title="Season Office", **season_office_context()),
    )
