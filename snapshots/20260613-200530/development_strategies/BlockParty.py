...
class BlockParty(IStrategy):
    '''
    Block Party (rev.4): cartographer laggard-catch-up strategy.
    Revision focus: loosened entry confirmation (broadened single-indicator
    criteria, added deep-laggard raw track, dual lookback 24h/4h constellation
    detection) to address zero-trade pacing reported in the C-grade shift.
    Preserves exit and management genomes; hardened plumbing.
    '''
    ...
    # --- Tunable parameters (loosened per C-grade review) --------------------
    lookback = 96               # 24h on 15m bars (slow constellation)
    lookback_fast = 16          # 4h on 15m bars (early expansion track)
    min_group_pct = 0.30
    group_move_min = 1.0        # lowered: 1.0% median move counts as lit
    group_move_fast_min = 0.6   # fast-window lit threshold
    group_move_strong = 3.0     # strong move threshold
    laggard_gap = 0.8           # lowered core gap
    soft_laggard_gap = 0.2      # lowered soft gap
    deep_laggard_gap = 1.8      # deep-value laggard track (no committee)
    min_breadth = 0.30          # lowered coherence
    group_exit_level = 0.4      # cool-off threshold
    rsi_overbought = 80
    rsi_oversold = 20
    vol_mult = 0.20             # volume floor relaxed
    early_catchup_pct = 0.15    # bar-move ignition threshold relaxed
