...
def init_db() -> None:
    ...
    with closing(get_db()) as conn:
        conn.executescript(
            ...
            -- Backtest Calibration Lab: one row per dry-run "live window" (the truth tape),
            -- paired to a matching backtest over the EXACT same window, scored by how
            -- faithfully the simulation reproduced live reality (not by P&L). Measure first.
            CREATE TABLE IF NOT EXISTS calibration_windows (
                window_id TEXT PRIMARY KEY,
                league_type TEXT NOT NULL DEFAULT 'dev',
                strategy_key TEXT NOT NULL DEFAULT '',
                strategy_name TEXT NOT NULL DEFAULT '',
                strategy_version TEXT NOT NULL DEFAULT '',
                universe_key TEXT NOT NULL DEFAULT '',
                live_exchange TEXT NOT NULL DEFAULT '',
                backtest_exchange TEXT NOT NULL DEFAULT '',
                timeframe TEXT NOT NULL DEFAULT '',
                ...
                live_pnl_pct REAL NOT NULL DEFAULT 0,
                live_win_rate REAL NOT NULL DEFAULT 0,
                live_avg_roi REAL NOT NULL DEFAULT 0,
                live_avg_hold_minutes REAL NOT NULL DEFAULT 0,
                live_max_drawdown REAL NOT NULL DEFAULT 0,
                live_exit_comp_json TEXT NOT NULL DEFAULT '{}',
                live_pairs_json TEXT NOT NULL DEFAULT '[]',
                live_short_share REAL NOT NULL DEFAULT 0,
                backtest_job_id TEXT NOT NULL DEFAULT '',
                backtest_run_id TEXT NOT NULL DEFAULT '',
                engine_version TEXT NOT NULL DEFAULT '',
                bt_metrics_json TEXT NOT NULL DEFAULT '{}',
                gap_metrics_json TEXT NOT NULL DEFAULT '{}',
                reality_score REAL NOT NULL DEFAULT 0,
                top_mismatch TEXT NOT NULL DEFAULT '',
                status TEXT NOT NULL DEFAULT 'pending',
                created_at TEXT NOT NULL DEFAULT '',
                scored_at TEXT
            );
            CREATE INDEX IF NOT EXISTS idx_calibration_windows_strategy ON calibration_windows (strategy_key, created_at);

            -- Live in-season strategy versioning: a season freezes HISTORY, not
            -- strategies. A "version" is one era/generation of a runtime (team_id);
            -- standings stay cumulative by strategy while analytics split by version.
            ...
            CREATE UNIQUE INDEX IF NOT EXISTS idx_strategy_versions_one_active
                ON strategy_versions(strategy_id) WHERE status = 'active';
            ...
            -- Competition Environment versioning — a SEPARATE axis from strategy
            -- versions. Captures the measurement conditions a runtime trades under
            -- (exchange, stake, universe construction, pairlist source). Lets us ask
            -- "did Timmy improve, or did the universe change?" Per-runtime, like
            -- strategy_versions; standings stay cumulative, analytics split by env era.
            CREATE TABLE IF NOT EXISTS environment_versions (
                ...
                stake_currency TEXT NOT NULL DEFAULT '',
                universe_label TEXT NOT NULL DEFAULT '',
                universe_construction TEXT NOT NULL DEFAULT '',
                pairlist_source TEXT NOT NULL DEFAULT '',
                pairlist_name TEXT NOT NULL DEFAULT '',
                number_assets INTEGER,
                change_type TEXT NOT NULL DEFAULT '',
                change_summary TEXT NOT NULL DEFAULT '',
                reason_for_change TEXT NOT NULL DEFAULT '',
                evidence_snapshot_json TEXT NOT NULL DEFAULT '{}',
            ...
            CREATE UNIQUE INDEX IF NOT EXISTS idx_environment_versions_one_active
                ON environment_versions(strategy_id) WHERE status = 'active';

            CREATE TABLE IF NOT EXISTS environment_version_events (
                id INTEGER PRIMARY KEY AUTOINCREMENT,
                event_type TEXT NOT NULL,
                strategy_id TEXT NOT NULL,
                old_version_id INTEGER,
        ...
        # Per-version trade attribution. opened_under is write-once (the version live
        # when the trade OPENED); managed_under is who actually ran the exit; the trade
        # is transition_managed when those differ (analytically mixed inventory).
        ensure_column(conn, "team_trades", "strategy_version_id", "INTEGER")
        ensure_column(conn, "team_trades", "managed_under_version_id", "INTEGER")
        ensure_column(conn, "team_trades", "transition_managed", "INTEGER NOT NULL DEFAULT 0")
        # Competition Environment attribution (parallel axis to strategy versions).
        ensure_column(conn, "team_trades", "environment_version_id", "INTEGER")
        ensure_column(conn, "team_trades", "managed_under_environment_version_id", "INTEGER")
        # Backtest Calibration Lab: job kind (health|habitat|paired_calibration), the engine
        # version that produced a run, validity status, and the live window a paired job mirrors.
        ensure_column(conn, "backtest_jobs", "job_kind", "TEXT NOT NULL DEFAULT 'health'")
        ensure_column(conn, "backtest_jobs", "engine_version", "TEXT NOT NULL DEFAULT ''")
        ensure_column(conn, "backtest_jobs", "live_window_id", "TEXT NOT NULL DEFAULT ''")
        ensure_column(conn, "backtest_results", "validity_status", "TEXT NOT NULL DEFAULT 'valid'")
        ensure_column(conn, "backtest_results", "validity_reason", "TEXT NOT NULL DEFAULT ''")
        ensure_column(conn, "backtest_results", "engine_version", "TEXT NOT NULL DEFAULT ''")
...
def seed_exchange_resources() -> None:
    ...
    setting_defaults = {
        ...
        # Backtest Calibration Lab cadence/window.
        "backtest_calibration_enabled": "true",
        "backtest_calibration_interval_minutes": "360",
        "backtest_calibration_window_days": "7",
    }
    with closing(get_db()) as conn:
...
def shift_exit_composition(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Forced-aware exit composition over a shift's closed trades. 'autonomous' = a
    strategy-CHOSEN non-stop exit (roi / exit_signal / custom profit tag) — the only way
    a strategy banks a winner on its own. Shared by the diagnostic, the grade and the UI
    so forced exits stop hiding and an exit-autonomy failure is measurable, not narrative."""
    closed = [r for r in records if not int(r.get("is_open") or 0)]
    n = len(closed)
    out = {"n": n, "stop": 0, "forced": 0, "autonomous": 0,
           "forced_winners": 0, "autonomous_winners": 0,
           "stop_share": 0.0, "forced_share": 0.0, "autonomous_share": 0.0}
    if n == 0:
        return out
    for r in closed:
        reason = str(r.get("exit_reason") or "").lower()
        forced = bool(int(r.get("forced") or 0)) or reason in DEV_FORCED_EXIT_REASONS
        win = parse_float(r.get("profit_abs")) > 0 or parse_float(r.get("profit_ratio")) > 0
        if forced:
            out["forced"] += 1
            out["forced_winners"] += 1 if win else 0
        elif reason in POST_SHIFT_STOP_EXITS:
            out["stop"] += 1
        else:
            out["autonomous"] += 1
            out["autonomous_winners"] += 1 if win else 0
    out["stop_share"] = out["stop"] / n
    out["forced_share"] = out["forced"] / n
    out["autonomous_share"] = out["autonomous"] / n
    return out


def exit_autonomy_failed(comp: dict[str, Any]) -> bool:
    """The pathology: enough closes to judge, autonomous (strategy-chosen non-stop) exits are
    rare, and the CLOCK banks at least as many winners as the strategy does — i.e. winners are
    realized by the shift bell, not by the strategy's own exit logic. A lone accidental roi/exit
    does not exempt a book that is otherwise all forced/stop."""
    return (int(comp.get("n") or 0) >= 4
            and parse_float(comp.get("autonomous_share")) < 0.25
            and int(comp.get("forced_winners") or 0) >= 1
            and int(comp.get("forced_winners") or 0) >= int(comp.get("autonomous_winners") or 0))
...
def diagnose_shift_trades(records: list[dict[str, Any]], style: str) -> list[dict[str, Any]]:
    ...
    fix is a concrete instruction the regenerator can act on. Most findings grade the
    strategy-CHOSEN exits (forced shift-bell exits excluded — the clock's doing), but the
    exit-autonomy finding is deliberately forced-AWARE so a strategy that only ever stops
    out or gets force-liquidated can no longer hide. Deterministic; no LLM."""
    ...
    # 0. Exit autonomy (forced-AWARE): does the strategy ever bank a winner on its own, or
    # does every win come from the shift bell? Computed over ALL closes, before the n==0
    # guard, so an all-forced/all-stop strategy is still indicted instead of returning clean.
    comp = shift_exit_composition(records)
    if exit_autonomy_failed(comp):
        findings.append({
            "code": "no_autonomous_exit", "severity": "critical",
            "headline": (
                f"No autonomous profit exit: {comp['autonomous']}/{comp['n']} closes were strategy-chosen "
                f"(non-stop); {comp['forced_winners']} winner(s) only realized at the shift bell and "
                f"{comp['stop']} closed on the stop. The strategy never banks a winner on its own — "
                f"a stale hypothesis is held until the clock force-exits it, blocking throughput."),
            "fix": (
                "Add a take-profit AND a stale-thesis exit: close profitable trades on your own signal, and "
                "sell a hypothesis that no longer holds so winners realize and the slot frees up. Verify "
                "custom_exit is actually reached — do not gate it behind an empty-dataframe early return."),
        })
...
def record_development_post_shift_review(
    ...
    # Forced-aware exit composition (computed once; reused for the grade penalty AND the
    # clinical findings below) so an exit-autonomy failure is graded, not just narrated.
    diagnostic_records = shift_diagnostic_records(candidate, started_at, stopped_at)
    exit_comp = shift_exit_composition(diagnostic_records)
    ...
    # Exit-autonomy penalty: PnL economics still grade on strategy-chosen exits (fair), but a
    # strategy that never banks a winner on its own — every win is the clock's — has a
    # non-functional exit and loses Economics points. Forced exits no longer hide from the grade.
    if exit_autonomy_failed(exit_comp):
        autonomy_penalty = 8.0
        profitability_score = max(0.0, round(profitability_score - autonomy_penalty, 1))
        profitability_note += (
            f" Exit-autonomy failure (−8): 0 of {exit_comp['n']} closes banked a winner on the strategy's "
            f"own signal ({exit_comp['forced_winners']} forced winner(s), {exit_comp['stop']} stop-outs, "
            f"{exit_comp['autonomous_share']:.0%} autonomous) — the exit logic never fires.")
    ...
    # (diagnostic_records was already gathered above for the exit-autonomy grade.)
    ...
    autonomy_clause = (
        f"Exit composition: {exit_comp['autonomous_share']:.0%} autonomous / {exit_comp['stop_share']:.0%} stop / "
        f"{exit_comp['forced_share']:.0%} forced — the strategy never banks a winner on its own. "
        if exit_autonomy_failed(exit_comp) else ""
    )
    summary = (
        ...
        f"{forced_clause}{autonomy_clause}"
...
def _backtest_strategy_payload(strategy_path: Path, key: str, display_name: str, timeframe: str) -> dict[str, Any]:
    ...
    # Resolve a freqtrade-legal timeframe. Candidate `timeframe` metadata is often LLM
    # prose (e.g. "15m base timeframe with confirmation from 1h 4h ...") which freqtrade
    # rejects as a --timeframe; the `timeframe = '...'` literal in the .py is the source
    # of truth. Prefer the declared literal, then a normalized token, then 5m.
    resolved_tf = (
        strategy_declared_timeframe({"strategy_path": str(strategy_path)})
        or normalize_freqtrade_timeframe(timeframe)
        or "5m"
    )
    return {
...
def read_full_backtest_stats(archive_filename: str) -> dict[str, Any]:
    """The FULL freqtrade strategy stats dict from a retained archive zip — the rich output
    (results_per_pair, results_per_enter_tag, exit_reason_summary, drawdown, sharpe/sortino/
    calmar, holding times, balances) that the summary record doesn't keep. {} if unavailable."""
    if not archive_filename:
        return {}
    zip_path = BACKTEST_DIR / archive_filename
    if not zip_path.exists():
        return {}
    try:
        with ZipFile(zip_path) as archive:
            inner = [n for n in archive.namelist() if n.endswith(".json") and "config" not in n and "market_change" not in n]
            if not inner:
                return {}
            data = json.loads(archive.read(inner[0]))
    except Exception:  # noqa: BLE001
        return {}
    strategies = data.get("strategy", {}) or {}
    if not strategies:
        return {}
    _, stats = next(iter(strategies.items()))
    return stats if isinstance(stats, dict) else {}


def _normalize_ft_group(rows: list[dict[str, Any]] | None) -> list[dict[str, Any]]:
    out: list[dict[str, Any]] = []
    for r in rows or []:
        tr = int(r.get("trades") or 0)
        wins = int(r.get("wins") or 0)
        mean_pct = r.get("profit_mean_pct")
        if mean_pct is None:
            mean_pct = parse_float(r.get("profit_mean")) * 100.0
        out.append({
            "key": r.get("key") or r.get("pair") or r.get("exit_reason") or r.get("enter_tag") or "(none)",
            "trades": tr, "wins": wins, "losses": int(r.get("losses") or 0), "draws": int(r.get("draws") or 0),
            "profit_total_abs": round(parse_float(r.get("profit_total_abs")), 3),
            "profit_mean_pct": round(parse_float(mean_pct), 3),
            "win_rate": round(100.0 * wins / tr, 1) if tr else 0.0,
        })
    return out


def _aggregate_trades_by(trades: list[dict[str, Any]], keyfn) -> list[dict[str, Any]]:
    groups: dict[str, dict[str, Any]] = {}
    for t in trades:
        k = str(keyfn(t) or "(none)")
        b = groups.setdefault(k, {"key": k, "trades": 0, "profit_total_abs": 0.0, "wins": 0, "_sum_pct": 0.0})
        b["trades"] += 1
        b["profit_total_abs"] += parse_float(t.get("profit_abs"))
        b["_sum_pct"] += parse_float(t.get("profit_ratio")) * 100.0
        if parse_float(t.get("profit_abs")) > 0:
            b["wins"] += 1
    out = []
    for b in groups.values():
        tr = b["trades"]
        out.append({"key": b["key"], "trades": tr, "wins": b["wins"], "losses": tr - b["wins"], "draws": 0,
                    "profit_total_abs": round(b["profit_total_abs"], 3),
                    "profit_mean_pct": round(b["_sum_pct"] / tr, 3) if tr else 0.0,
                    "win_rate": round(100.0 * b["wins"] / tr, 1) if tr else 0.0})
    return sorted(out, key=lambda x: x["profit_total_abs"], reverse=True)


# Freqtrade summary stats worth surfacing on the inspection page (label, key, kind).
_BACKTEST_SUMMARY_FIELDS = [
    ("Total trades", "total_trades", "int"), ("Total profit", "profit_total_abs", "f2"),
    ("Total profit %", "profit_total", "pct"), ("Win rate", "winrate", "pct"),
    ("Profit factor", "profit_factor", "f2"), ("Expectancy ratio", "expectancy_ratio", "f2"),
    ("CAGR", "cagr", "pct"), ("Sharpe", "sharpe", "f2"), ("Sortino", "sortino", "f2"),
    ("Calmar", "calmar", "f2"), ("Max drawdown (acct)", "max_drawdown_account", "pct"),
    ("Market change", "market_change", "pct"), ("Starting balance", "starting_balance", "f2"),
    ("Final balance", "final_balance", "f2"), ("Trades/day", "trades_per_day", "f2"),
    ("Avg duration", "holding_avg", "str"), ("Winner avg dur", "winner_holding_avg", "str"),
    ("Loser avg dur", "loser_holding_avg", "str"), ("Winning days", "winning_days", "int"),
    ("Draw days", "draw_days", "int"), ("Losing days", "losing_days", "int"),
]


def backtest_inspection(record: dict[str, Any]) -> dict[str, Any]:
    """Build the rich, freqtrade-style inspection for a single backtest run: full summary stats,
    per-pair / per-enter-tag / exit-reason tables, and the full trades list. Uses the retained
    archive zip when present; falls back to aggregating the run record's stored trades."""
    stats = read_full_backtest_stats(str(record.get("archive") or ""))
    trades = (stats.get("trades") if stats else None) or record.get("trades") or []
    summary: list[dict[str, Any]] = []
    if stats:
        for label, key, kind in _BACKTEST_SUMMARY_FIELDS:
            if key not in stats:
                continue
            v = stats.get(key)
            if kind == "pct":
                txt = f"{parse_float(v) * 100.0:+.2f}%" if abs(parse_float(v)) <= 5 else f"{parse_float(v):+.2f}%"
            elif kind == "f2":
                txt = f"{parse_float(v):.2f}"
            elif kind == "int":
                txt = str(int(parse_float(v)))
            else:
                txt = str(v)
            summary.append({"label": label, "value": txt})
    per_pair = _normalize_ft_group(stats.get("results_per_pair")) if stats.get("results_per_pair") else _aggregate_trades_by(trades, lambda t: t.get("pair"))
    per_enter = _normalize_ft_group(stats.get("results_per_enter_tag")) if stats.get("results_per_enter_tag") else _aggregate_trades_by(trades, lambda t: t.get("enter_tag"))
    exit_summary = _normalize_ft_group(stats.get("exit_reason_summary")) if stats.get("exit_reason_summary") else _aggregate_trades_by(trades, lambda t: t.get("exit_reason"))
    # Cap the rendered trade rows — a 1m strategy can produce tens of thousands.
    trade_cap = 1000
    trade_rows = [{
        "pair": t.get("pair"), "side": "short" if int(t.get("is_short") or 0) else "long",
        "enter_tag": t.get("enter_tag") or "", "exit_reason": t.get("exit_reason") or "",
        "open_date": str(t.get("open_date") or "")[:16], "close_date": str(t.get("close_date") or "")[:16],
        "duration_min": round(parse_float(t.get("trade_duration")) or parse_float(t.get("trade_duration_minutes")), 1),
        "profit_pct": round(parse_float(t.get("profit_ratio")) * 100.0, 3),
        "profit_abs": round(parse_float(t.get("profit_abs")), 4),
        "open_rate": t.get("open_rate"), "close_rate": t.get("close_rate"),
    } for t in trades[:trade_cap]]
    return {"has_archive": bool(stats), "summary": summary, "per_pair": per_pair,
            "per_enter_tag": per_enter, "exit_summary": exit_summary, "trades": trade_rows,
            "trades_total": len(trades), "trades_truncated": len(trades) > trade_cap}
...
def ensure_backtest_ohlcv(exchange: str, market_type: str, quote: str, timeframe: str, min_pairs: int = 5) -> None:
    """Self-heal: make sure local OHLCV exists at `timeframe` for the broad pool before
    a backtest needs it. The department only pre-downloaded 1h/5m, so strategies on 15m,
    1m, 4h, 1d, ... hit 'No data found. Terminating.' This downloads the missing
    timeframe on demand (cached afterwards). Best-effort and gated by a setting."""
    if get_setting("backtest_autodownload_enabled", "true").strip().lower() != "true":
        return
    if not timeframe or timeframe not in FREQTRADE_TIMEFRAMES:
        return
    have = _local_ohlcv_pairs(exchange, market_type, timeframe)
    if len(have) >= min_pairs:
        return
    try:
        days = DEPARTMENT_WINDOW_DAYS + 10
        result = download_universe_pool(exchange, market_type, quote, timeframes=(timeframe,), days=days)
        log_maintenance(
            "backtesting", "ok" if result.get("ok") else "warning",
            f"Auto-downloaded {exchange} {timeframe} OHLCV "
            f"({result.get('pairs_local_after', 0)} pairs local).",
        )
    except Exception as exc:  # noqa: BLE001
        log_maintenance("backtesting", "warning", f"Auto-download {exchange}/{timeframe} failed: {exc}")


def strategy_informative_timeframes(strategy_path: str) -> list[str]:
    """Timeframes a strategy pulls as informative pairs (e.g. @informative('1d')), so the
    backtest pre-downloads them too — otherwise freqtrade aborts with 'Informative
    dataframe for (PAIR, 1d, futures) is empty. Can't populate informative indicators.'"""
    p = resolve_path(strategy_path)
    ...
    try:
        ...
        return []
    out: list[str] = []
    for tf in re.findall(r"@informative\(\s*['\"]([0-9]+[a-zA-Z])['\"]", text):
        norm = normalize_freqtrade_timeframe(tf)
...
def run_universe_backtest(
    ...
    timeframe = normalize_freqtrade_timeframe(timeframe) or strat["timeframe"]
    quote = backtest_quote_for(exchange)
    start_dt, end_dt = parse_timerange(timerange)
    # Self-heal local data: membership reconstruction reads 1h; the backtest itself reads
    # the strategy timeframe. Download either if missing so we don't fail with empty
    # union_pairs or freqtrade's "No data found. Terminating."
    ensure_backtest_ohlcv(exchange, market_type, quote, "1h")
    ensure_backtest_ohlcv(exchange, market_type, quote, timeframe)
    # Strategies with @informative('1d'/'4h'/...) decorators need those timeframes too.
    for inf_tf in strategy_informative_timeframes(strat.get("strategy_path", "")):
        ensure_backtest_ohlcv(exchange, market_type, quote, inf_tf)
    ...
    # Reconcile pricing with the strategy's declared order types: freqtrade refuses to run
    # a market-order strategy whose matching *_pricing.price_side is "same" (the base
    # config default) — "Market exit orders require exit_pricing.price_side = 'other'".
    _strat_cand = {"strategy_path": strat.get("strategy_path", "")}
    if strategy_declared_order_type(_strat_cand, "exit") == "market":
        cfg.setdefault("exit_pricing", {})["price_side"] = "other"
    if strategy_declared_order_type(_strat_cand, "entry") == "market":
...
# Backtest Calibration Lab: the simulator's own version. Bump ONLY when backtest
# execution behavior changes; every result is stamped with it so we can score whether
# an engine change raised live-correlation across organisms (doctrine: measure first).
...
def active_organisms() -> list[dict[str, Any]]:
    ...
    for cand in development_candidate_rows():
        ...
        out.append({
            ...
            "kind": "dev",
            "strategy_path": str(strategy_path),
            # Map the candidate's raw (often LLM-authored) coin_universe onto a real
            # canonical universe; using universe_key() let junk strings like
            # "custombasketbtcusdtplustop20comparisonuniverse" or "top10" through, which
            # then failed every health-check backtest with "no pairs reconstructed".
            "canonical_universe": canonical_universe_for_candidate(cand),
        })
    return out
...
def _calibration_job_pending(live_window_id: str) -> bool:
    with closing(get_db()) as conn:
        row = conn.execute(
            "SELECT 1 FROM backtest_jobs WHERE live_window_id=? AND status IN ('queued','running') LIMIT 1",
            (live_window_id,),
        ).fetchone()
    return bool(row)
...
def enqueue_backtest_job(
    ...
    """Insert a queued job unless a duplicate is already queued/running. Health/habitat jobs
    dedupe on (strategy, universe); paired_calibration jobs dedupe on live_window_id (each
    mirrors one exact live window). Returns the job_id, or None if deduped."""
    if job_kind == "paired_calibration":
        if not live_window_id or _calibration_job_pending(live_window_id):
            return None
    elif _backtest_job_queued_or_running(strategy_key, universe):
        return None
    universe_name = universe_label(universe)
    ...
    with closing(get_db()) as conn:
        conn.execute(
            ...
            INSERT INTO backtest_jobs (
                ...
                reason, comparison_group_id, created_at, job_kind, engine_version, live_window_id
            ) VALUES (?, ?, 'queued', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                job_id, title or f"{strategy_name} × {universe_name}", bucket, score, strategy_key,
                strategy_name, strategy_version, universe, universe_name, (exchange or DEPARTMENT_EXCHANGE),
                timeframe, timerange, mode, reason, comparison_group_id, iso_now(),
                job_kind, BACKTEST_ENGINE_VERSION, live_window_id,
            ),
        )
        conn.commit()
    return job_id
...
def complete_backtest_job(job: dict[str, Any], record: dict[str, Any]) -> None:
    ...
    validity_status, validity_reason = compute_backtest_validity(job, record)
    with closing(get_db()) as conn:
        if record.get("ok"):
            summary = (
            ...
            conn.execute(
                ...
                INSERT OR REPLACE INTO backtest_results (
                    ...
                    validity_status, validity_reason, engine_version
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    ...
                    validity_status, validity_reason, BACKTEST_ENGINE_VERSION,
                ),
            )
        else:
        ...
        conn.commit()
    # Paired-calibration jobs feed the Calibration Lab: score the live-vs-backtest gap.
    if str(job.get("job_kind") or "") == "paired_calibration" and job.get("live_window_id"):
        try:
            score_calibration_window(str(job["live_window_id"]), job, record)
        except Exception as exc:  # noqa: BLE001
            log_maintenance("backtesting", "warning", f"Calibration scoring failed for {job.get('live_window_id')}: {exc}")


# ============================================================
# Backtest Calibration Lab
#
# Doctrine: a backtest is judged not by P&L but by how faithfully it reproduces
# dry-run (live) reality. For each closed live window (a dev shift, or a rolling
# window for a continuous major-league bot) we snapshot the live truth, run a
# paired backtest over the EXACT same window, and score the reality gap. We also
# flag structurally-invalid runs (e.g. custom_exit inactive) so they cannot
# pollute the strategy leaderboard. Measure first, modify second.
# ============================================================

def calibration_enabled() -> bool:
    return get_setting("backtest_calibration_enabled", "true").strip().lower() == "true"


def _yyyymmdd(value: Any) -> str:
    dt = value if isinstance(value, datetime) else _version_dt(value)
    return dt.strftime("%Y%m%d") if dt else ""


def _window_timerange(start: Any, end: Any) -> str:
    """YYYYMMDD-YYYYMMDD covering [start, end]; widen the end by a day when the window
    falls inside a single calendar day so freqtrade's day-granular timerange isn't empty."""
    s = _version_dt(start)
    e = _version_dt(end) or s
    if not s:
        return ""
    if not e or e.date() <= s.date():
        e = s + timedelta(days=1)
    return f"{s.strftime('%Y%m%d')}-{e.strftime('%Y%m%d')}"


def _exit_distribution_composition(dist: dict[str, Any]) -> dict[str, Any]:
    """Bucket a {exit_tag: count} distribution into stop/forced/autonomous shares, the
    same way shift_exit_composition buckets per-trade records (autonomous = strategy-chosen
    non-stop exit: roi / exit_signal / custom tags)."""
    stop = forced = auto = 0
    for tag, cnt in (dist or {}).items():
        t = str(tag).lower()
        c = int(cnt or 0)
        if t in DEV_FORCED_EXIT_REASONS:
            forced += c
        elif t in POST_SHIFT_STOP_EXITS:
            stop += c
        else:
            auto += c
    n = stop + forced + auto
    return {"n": n, "stop": stop, "forced": forced, "autonomous": auto,
            "stop_share": stop / n if n else 0.0, "forced_share": forced / n if n else 0.0,
            "autonomous_share": auto / n if n else 0.0}


def compute_backtest_validity(job: dict[str, Any], record: dict[str, Any]) -> tuple[str, str]:
    """Classify a finished backtest BEFORE it can enter standings. The headline case:
    a strategy with a custom_exit but use_exit_signal=False can never fire custom_exit, so
    a run with zero autonomous exit tags is structurally INVALID (the Timmy pollution)."""
    if not record.get("ok"):
        return "failed", str(record.get("error") or "no archive produced")[:200]
    spath = ""
    try:
        spath = str(resolve_backtest_strategy(str(job.get("strategy_key") or "")).get("strategy_path") or "")
    except Exception:  # noqa: BLE001
        spath = ""
    if spath and strategy_has_custom_exit(spath) and strategy_uses_exit_signal(spath) is False:
        comp = _exit_distribution_composition(record.get("exit_tag_distribution", {}) or {})
        if comp["autonomous"] == 0:
            return "invalid", "custom_exit inactive (use_exit_signal=False): no strategy-chosen exits could fire"
    if str(job.get("job_kind") or "") == "paired_calibration":
        return "calibration_only", "paired calibration run (simulator scoring, not strategy ranking)"
    if int(record.get("total_trades", 0) or 0) < 5:
        return "suspect", f"thin sample ({int(record.get('total_trades', 0) or 0)} trades)"
    return "valid", ""


def _max_drawdown_pct_from_trades(closed: list[dict[str, Any]]) -> float:
    """Approximate per-window max drawdown (%) from a stake-normalized running equity over
    the closed trades in close-date order. Coarse but comparable to the backtest's DD%."""
    ordered = sorted(closed, key=lambda t: str(t.get("close_date") or t.get("open_date") or ""))
    equity = 0.0
    peak = 0.0
    max_dd = 0.0
    for t in ordered:
        equity += parse_float(t.get("profit_ratio")) * 100.0
        peak = max(peak, equity)
        max_dd = min(max_dd, equity - peak)
    return abs(max_dd)


def _calibration_live_metrics(trades: list[dict[str, Any]]) -> dict[str, Any]:
    """Live metric snapshot from a list of trade records (dev_archived_trades or team_trades)."""
    closed = [t for t in trades if not int(t.get("is_open") or 0)]
    n = len(closed)
    pnl = sum(parse_float(t.get("profit_abs")) for t in closed)
    rois = [parse_float(t.get("profit_ratio")) for t in closed]
    wins = sum(1 for t in closed if parse_float(t.get("profit_abs")) > 0)
    holds: list[float] = []
    for t in closed:
        d = parse_float(t.get("trade_duration_minutes"))
        if d <= 0:
            o, c = _ml_parse_dt(t.get("open_date")), _ml_parse_dt(t.get("close_date"))
            d = (c - o).total_seconds() / 60.0 if o and c else 0.0
        if d > 0:
            holds.append(d)
    comp = shift_exit_composition(closed)
    pairs = sorted({str(t.get("pair") or "") for t in closed if t.get("pair")})
    shorts = sum(1 for t in closed if int(t.get("is_short") or 0))
    return {
        "live_trades": n,
        "live_pnl": round(pnl, 4),
        "live_pnl_pct": round(sum(rois) * 100.0, 4),
        "live_win_rate": round(percentage(wins, n) * 100.0, 2),
        "live_avg_roi": round(stat_mean(rois) * 100.0, 4),
        "live_avg_hold_minutes": round(stat_mean(holds), 1),
        "live_max_drawdown": round(_max_drawdown_pct_from_trades(closed), 4),
        "live_exit_comp_json": json.dumps({k: comp[k] for k in ("stop_share", "forced_share", "autonomous_share", "n")}),
        "live_pairs_json": json.dumps(pairs),
        "live_short_share": round(percentage(shorts, n), 4),
    }


def _upsert_calibration_window(window_id: str, fields: dict[str, Any]) -> None:
    cols = list(fields.keys())
    with closing(get_db()) as conn:
        exists = conn.execute("SELECT 1 FROM calibration_windows WHERE window_id=?", (window_id,)).fetchone()
        if exists:
            conn.execute(f"UPDATE calibration_windows SET {', '.join(f'{c}=?' for c in cols)} WHERE window_id=?",
                         (*[fields[c] for c in cols], window_id))
        else:
            allcols = ["window_id"] + cols
            conn.execute(f"INSERT INTO calibration_windows ({', '.join(allcols)}) VALUES ({', '.join('?' * len(allcols))})",
                         (window_id, *[fields[c] for c in cols]))
        conn.commit()


def _calibration_window_exists(window_id: str) -> bool:
    with closing(get_db()) as conn:
        return bool(conn.execute("SELECT 1 FROM calibration_windows WHERE window_id=?", (window_id,)).fetchone())


def _major_universe_key(instance: dict[str, Any]) -> str:
    label = str(instance.get("pair_universe") or "")
    digits = "".join(ch for ch in label if ch.isdigit())
    size = int(digits) if digits else 50
    return "top20_marketcap" if size <= 20 else "top50_marketcap" if size <= 50 else "top100_marketcap"


def capture_and_pair_calibration_window(*, window_id: str, league_type: str, strategy_key: str,
                                        strategy_name: str, universe: str, timeframe: str,
                                        live_exchange: str, window_start: str, window_end: str,
                                        timerange: str, trades: list[dict[str, Any]],
                                        episode_key: str = "", config_hash: str = "",
                                        strategy_version: str = "") -> str | None:
    """Snapshot a live window's truth into calibration_windows and enqueue the paired
    backtest (department job kind 'paired_calibration'). Returns the job_id or None."""
    metrics = _calibration_live_metrics(trades)
    fields = {
        "league_type": league_type, "strategy_key": strategy_key, "strategy_name": strategy_name,
        "strategy_version": strategy_version, "universe_key": universe, "live_exchange": live_exchange,
        "backtest_exchange": DEPARTMENT_EXCHANGE, "timeframe": timeframe, "config_hash": config_hash,
        "window_start": window_start, "window_end": window_end, "timerange": timerange,
        "episode_key": episode_key, "engine_version": BACKTEST_ENGINE_VERSION,
        "status": "queued", "created_at": iso_now(), **metrics,
    }
    _upsert_calibration_window(window_id, fields)
    job_id = enqueue_backtest_job(
        strategy_key, strategy_name, strategy_version, universe, bucket=3, score=80.0,
        reason=f"Paired calibration vs live window {window_id}", timerange=timerange, timeframe=timeframe,
        title=f"Calibrate {strategy_name} ({timerange})", job_kind="paired_calibration",
        live_window_id=window_id, exchange=DEPARTMENT_EXCHANGE,
    )
    if job_id:
        _upsert_calibration_window(window_id, {"backtest_job_id": job_id})
    return job_id


def run_backtest_calibration_cycle() -> dict[str, Any]:
    """Enqueue paired-calibration backtests for (a) recent dev shift episodes not yet paired
    and (b) the current fixed rolling window of each major-league runtime. Automatic; runs
    on the maintenance cadence. Never modifies live trading or engine behavior."""
    if not calibration_enabled():
        return {"ok": False, "reason": "disabled"}
    paired = 0
    # (a) Dev shifts — each archived episode is a clean dry-run window.
    with closing(get_db()) as conn:
        episodes = [dict(r) for r in conn.execute(
            "SELECT * FROM dev_shift_episodes WHERE closed_trades > 0 ORDER BY id DESC LIMIT 20"
        ).fetchall()]
    for ep in episodes:
        window_id = f"dev:{ep['episode_key']}"
        if _calibration_window_exists(window_id):
            continue
        slug = str(ep.get("slug") or "")
        cand = next((c for c in development_candidate_rows() if str(c.get("slug")) == slug), None)
        if not cand:
            continue
        try:
            strat = resolve_backtest_strategy(f"dev:{slug}")
        except Exception:  # noqa: BLE001
            continue
        with closing(get_db()) as conn:
            trades = [dict(r) for r in conn.execute(
                "SELECT * FROM dev_archived_trades WHERE episode_key=?", (ep["episode_key"],)).fetchall()]
        if not trades:
            continue
        tr = _window_timerange(ep.get("session_started_at"), ep.get("session_stopped_at"))
        if not tr:
            continue
        if capture_and_pair_calibration_window(
            window_id=window_id, league_type="dev", strategy_key=f"dev:{slug}",
            strategy_name=str(ep.get("name") or slug), universe=canonical_universe_for_candidate(cand),
            timeframe=str(strat.get("timeframe") or "5m"), live_exchange=str(cand.get("exchange") or ""),
            window_start=str(ep.get("session_started_at") or ""), window_end=str(ep.get("session_stopped_at") or ""),
            timerange=tr, trades=trades, episode_key=str(ep["episode_key"]),
            strategy_version=str(ep.get("strategy_version") or ""),
        ):
            paired += 1
    # (b) Major league — a fixed rolling window per runtime.
    window_days = int(parse_float(get_setting("backtest_calibration_window_days", "7")) or 7)
    full_tr = department_default_timerange(days=window_days)
    start_s, end_s = full_tr.split("-", 1)
    win_start = datetime.strptime(start_s, "%Y%m%d").replace(tzinfo=UTC)
    win_end = datetime.strptime(end_s, "%Y%m%d").replace(tzinfo=UTC)
    for instance in list_instances():
        team_id = str(instance.get("id") or "")
        window_id = f"major:{team_id}:{full_tr}"
        if not team_id or _calibration_window_exists(window_id):
            continue
        with closing(get_db()) as conn:
            rows = [dict(r) for r in conn.execute(
                "SELECT * FROM team_trades WHERE team_id=? AND is_open=0 AND close_date >= ? AND close_date <= ?",
                (team_id, win_start.isoformat(), (win_end + timedelta(days=1)).isoformat())).fetchall()]
        if not rows:
            continue
        try:
            strat = resolve_backtest_strategy(team_id)
            tf = str(strat.get("timeframe") or "5m")
        except Exception:  # noqa: BLE001
            tf = "5m"
        active = active_strategy_version(team_id)
        if capture_and_pair_calibration_window(
            window_id=window_id, league_type="major", strategy_key=team_id,
            strategy_name=str(instance.get("display_name") or team_id), universe=_major_universe_key(instance),
            timeframe=tf, live_exchange=str((load_json(resolve_path(instance.get('config_path')), {}) or {}).get('exchange', {}).get('name') or ''),
            window_start=win_start.isoformat(), window_end=win_end.isoformat(), timerange=full_tr,
            trades=rows, strategy_version=str((active or {}).get("version_label") or ""),
        ):
            paired += 1
    set_setting("backtest_calibration_last_run", iso_now())
    return {"ok": True, "paired": paired}


def score_calibration_window(window_id: str, job: dict[str, Any], record: dict[str, Any]) -> None:
    """Compute the reality gap between the stored live snapshot and the paired backtest, and
    persist the per-metric deltas + a 0-100 reality_score (higher = backtest reproduced live)."""
    with closing(get_db()) as conn:
        row = conn.execute("SELECT * FROM calibration_windows WHERE window_id=?", (window_id,)).fetchone()
    if not row:
        return
    w = dict(row)
    if not record.get("ok"):
        _upsert_calibration_window(window_id, {"status": "failed", "scored_at": iso_now(),
                                               "backtest_run_id": record.get("run_id", "")})
        return
    live_n = int(w.get("live_trades") or 0)
    bt_n = int(record.get("total_trades", 0) or 0)
    live_comp = decode_jsonish_payload(str(w.get("live_exit_comp_json") or "{}"), {})
    bt_comp = _exit_distribution_composition(record.get("exit_tag_distribution", {}) or {})
    bt_pairs = sorted({str(t.get("pair") or "") for t in (record.get("trades") or []) if t.get("pair")})
    live_pairs = decode_jsonish_payload(str(w.get("live_pairs_json") or "[]"), [])
    bt_shorts = sum(1 for t in (record.get("trades") or []) if int(t.get("is_short") or 0))
    bt_short_share = (bt_shorts / bt_n) if bt_n else 0.0

    def sim(a: float, b: float, scale: float) -> float:  # 1.0 when equal, decaying with |a-b|/scale
        return max(0.0, 1.0 - abs(parse_float(a) - parse_float(b)) / scale) if scale else 0.0

    exit_sim = 1.0 - 0.5 * sum(abs(parse_float(live_comp.get(k)) - bt_comp[k])
                               for k in ("stop_share", "forced_share", "autonomous_share"))
    trade_count_ratio = (min(live_n, bt_n) / max(live_n, bt_n)) if max(live_n, bt_n) else 0.0
    pair_overlap = (len(set(live_pairs) & set(bt_pairs)) / len(set(live_pairs) | set(bt_pairs))) if (live_pairs or bt_pairs) else 1.0
    dir_overlap = 1.0 - abs(parse_float(w.get("live_short_share")) - bt_short_share)
    win_sim = sim(w.get("live_win_rate"), record.get("winrate_pct"), 100.0)
    roi_sim = sim(w.get("live_avg_roi"), record.get("avg_roi_pct"), 5.0)
    dd_sim = sim(w.get("live_max_drawdown"), record.get("max_drawdown_pct"), 25.0)
    hold_sim = sim(w.get("live_avg_hold_minutes"), record.get("avg_hold_minutes"),
                   max(60.0, parse_float(w.get("live_avg_hold_minutes")) or 60.0))

    components = {
        "trade_count_ratio": round(trade_count_ratio, 3), "exit_mix_similarity": round(max(0.0, exit_sim), 3),
        "win_rate_similarity": round(win_sim, 3), "avg_roi_similarity": round(roi_sim, 3),
        "max_dd_similarity": round(dd_sim, 3), "hold_time_similarity": round(hold_sim, 3),
        "pair_overlap": round(pair_overlap, 3), "direction_overlap": round(max(0.0, dir_overlap), 3),
    }
    weights = {"trade_count_ratio": 1.5, "exit_mix_similarity": 2.0, "win_rate_similarity": 1.5,
               "avg_roi_similarity": 1.0, "max_dd_similarity": 1.0, "hold_time_similarity": 1.0,
               "pair_overlap": 1.0, "direction_overlap": 1.0}
    reality_score = round(100.0 * sum(components[k] * weights[k] for k in components) / sum(weights.values()), 1)
    top_mismatch = min(components.items(), key=lambda kv: kv[1])[0]
    gap_metrics = {**components, "pnl_delta": round(parse_float(record.get("profit_total_abs")) - parse_float(w.get("live_pnl")), 4),
                   "live_trades": live_n, "backtest_trades": bt_n}
    bt_metrics = {"trades": bt_n, "pnl": round(parse_float(record.get("profit_total_abs")), 4),
                  "pnl_pct": round(parse_float(record.get("profit_total_pct")), 3),
                  "win_rate": round(parse_float(record.get("winrate_pct")), 2),
                  "avg_roi": round(parse_float(record.get("avg_roi_pct")), 3),
                  "avg_hold_minutes": round(parse_float(record.get("avg_hold_minutes")), 1),
                  "max_drawdown": round(parse_float(record.get("max_drawdown_pct")), 3),
                  "exit_comp": {k: bt_comp[k] for k in ("stop_share", "forced_share", "autonomous_share", "n")}}
    _upsert_calibration_window(window_id, {
        "backtest_run_id": record.get("run_id", ""), "engine_version": BACKTEST_ENGINE_VERSION,
        "bt_metrics_json": json.dumps(bt_metrics), "gap_metrics_json": json.dumps(gap_metrics),
        "reality_score": reality_score, "top_mismatch": top_mismatch, "status": "scored", "scored_at": iso_now(),
    })


def engine_calibration_summary() -> list[dict[str, Any]]:
    with closing(get_db()) as conn:
        rows = conn.execute(
            "SELECT engine_version, COUNT(*) n, AVG(reality_score) avg_score FROM calibration_windows "
            "WHERE status='scored' GROUP BY engine_version ORDER BY engine_version DESC"
        ).fetchall()
    return [{"engine_version": r["engine_version"] or "?", "windows": int(r["n"]),
             "avg_reality_score": round(parse_float(r["avg_score"]), 1)} for r in rows]


def calibration_windows_recent(limit: int = 60) -> list[dict[str, Any]]:
    with closing(get_db()) as conn:
        rows = [dict(r) for r in conn.execute(
            "SELECT * FROM calibration_windows ORDER BY created_at DESC LIMIT ?", (limit,)).fetchall()]
    for r in rows:
        r["gap_metrics"] = decode_jsonish_payload(str(r.get("gap_metrics_json") or "{}"), {})
        r["bt_metrics"] = decode_jsonish_payload(str(r.get("bt_metrics_json") or "{}"), {})
        r["live_exit_comp"] = decode_jsonish_payload(str(r.get("live_exit_comp_json") or "{}"), {})
    return rows
...
def reclaim_orphaned_backtests(force_all: bool = False, max_age_seconds: int = 3000) -> int:
    """Release lanes stuck in 'running' and requeue their job. A backtest runs inside the
    department loop's process, so if the app is restarted/crashes mid-run, complete_backtest_job
    never fires and the lane stays 'running' forever — run_department_once then refuses to claim
    any new job and the whole department freezes. `force_all=True` (startup) reclaims every
    running lane since nothing is actually running yet; otherwise only lanes older than
    max_age_seconds (> the 40-min backtest timeout) are reclaimed, so a genuinely in-flight
    backtest is never killed."""
    now = utc_now()
    reclaimed = 0
    with closing(get_db()) as conn:
        for lane in [dict(r) for r in conn.execute("SELECT * FROM backtest_lanes WHERE status = 'running'").fetchall()]:
            started = normalize_utc(resolve_optional_datetime(str(lane.get("started_at") or "")))
            age = (now - started).total_seconds() if started else 1e9
            if not force_all and age < max_age_seconds:
                continue
            job_id = str(lane.get("current_job_id") or "")
            if job_id:
                conn.execute(
                    "UPDATE backtest_jobs SET status='queued', started_at=NULL, lane_id='' "
                    "WHERE job_id=? AND status='running'",
                    (job_id,),
                )
            conn.execute(
                "UPDATE backtest_lanes SET status='idle', current_job_id='', started_at=NULL, "
                "failure_reason='reclaimed stale lock (app restart / hang)', updated_at=? WHERE lane_id=?",
                (iso_now(), lane["lane_id"]),
            )
            reclaimed += 1
        # Also requeue any orphaned 'running' job whose lane is no longer holding it.
        conn.execute(
            "UPDATE backtest_jobs SET status='queued', started_at=NULL, lane_id='' WHERE status='running' "
            "AND job_id NOT IN (SELECT current_job_id FROM backtest_lanes WHERE status='running')"
        )
        conn.commit()
    if reclaimed:
        log_maintenance("backtesting", "warning", f"Reclaimed {reclaimed} stale backtest lane(s) and requeued the job(s).")
    return reclaimed
    ...
    while True:
        try:
            if department_enabled() and not department_paused() and _department_gap_elapsed():
                reclaim_orphaned_backtests()  # self-heal a lane left 'running' by a crash/hang
...
def _read_strategy_text(strategy_path: str | None) -> str:
    path = resolve_path(strategy_path)
    if not path or not path.exists():
        return ""
    try:
...
def strategy_uses_exit_signal(strategy_path: str | None) -> bool | None:
    """The `use_exit_signal = True/False` class flag, or None if not declared (freqtrade
    defaults to True). CRITICAL: freqtrade only evaluates custom_exit inside
    `if self.use_exit_signal:` — False silently disables the entire custom_exit (the Timmy
    bug). Used to flag backtests whose custom exits could not have fired as INVALID."""
    text = _read_strategy_text(strategy_path)
    m = re.search(r"^\s*use_exit_signal\s*=\s*(True|False)\b", text, re.MULTILINE)
    if not m:
        return None
    return m.group(1) == "True"


def strategy_has_custom_exit(strategy_path: str | None) -> bool:
    """Whether the strategy defines a custom_exit method it relies on for exits."""
    text = _read_strategy_text(strategy_path)
...
ML_TELEMETRY_CATEGORIES: list[dict[str, Any]] = [
    ...
    # Share of closes the strategy CHOSE on its own (non-stop, non-forced) — how often it
    # banks a winner itself vs only stopping out or being force-liquidated at the bell. Low
    # exit_autonomy is the dany/second-act pathology (never sells a stale-but-green hypothesis).
    {"name": "exit_autonomy", "good": "high", "measurable": True},
...
def seed_default_settings() -> None:
    ...
    defaults = {
        ...
        "ollama_breaker_threshold": "4",
        "ollama_breaker_cooldown_minutes": "15",
...
def upsert_trade_rows(team_id: str, source_db_path: str, trades: list[dict[str, Any]]) -> None:
    ...
    with closing(get_db()) as conn:
        ...
        conn.commit()
    # Attribute the freshly-synced rows to a version era. opened_under is write-once
    # (closed trades keep their open-era version); managed_under/transition refresh here.
    try:
        attribute_trade_versions(team_id)
        attribute_trade_environments(team_id)
    except Exception as exc:  # noqa: BLE001
        log_maintenance("versioning", "warning", f"Trade version attribution failed for {team_id}: {exc}")
        ...
        rankings.append(
            {
                ...
                "score": round(score, 1),
                **version_power_read(instance["id"]),
            }
        )
...
# ---------------------------------------------------------------------------
# Live in-season strategy versioning.
#
# A "strategy version" is one era/generation of a *runtime* (team_id). Major
# league runs the same strategy file as separate bots (Timmy Top 50 vs Top 20),
# so versions are scoped per team_id: each runtime has its own lineage and its
# own single active version. Trades attribute to the version live when they
# OPENED (write-once); standings stay cumulative by strategy. See the approved
# plan for the full conceptual model.
# ---------------------------------------------------------------------------

VERSION_CHANGE_TYPES = {
    "launch": "Launch",
    "exit_system_revision": "Exit system revision",
    "entry_logic": "Entry logic change",
    "timeframe": "Timeframe change",
    "position_sizing": "Position sizing change",
    "coin_universe": "Coin universe change",
    "risk_regime": "Stop-loss / take-profit regime",
    "signal_threshold": "Signal threshold change",
    "other": "Other major change",
}


def _version_row_dict(row: Any) -> dict[str, Any]:
    item = dict(row)
    item["evidence_snapshot"] = decode_jsonish_payload(str(item.get("evidence_snapshot_json") or "{}"), {})
    item["config_snapshot"] = decode_jsonish_payload(str(item.get("config_snapshot_json") or "{}"), {})
    item["change_type_label"] = VERSION_CHANGE_TYPES.get(str(item.get("change_type") or ""), str(item.get("change_type") or "").replace("_", " ").title())
    return item


def strategy_versions_for(strategy_id: str) -> list[dict[str, Any]]:
    """All versions for a runtime, oldest version_number first."""
    with closing(get_db()) as conn:
        rows = conn.execute(
            "SELECT * FROM strategy_versions WHERE strategy_id = ? ORDER BY version_number ASC",
            (strategy_id,),
        ).fetchall()
    return [_version_row_dict(row) for row in rows]


def strategy_version_by_id(version_id: int) -> dict[str, Any] | None:
    with closing(get_db()) as conn:
        row = conn.execute("SELECT * FROM strategy_versions WHERE id = ?", (int(version_id),)).fetchone()
    return _version_row_dict(row) if row else None


def active_strategy_version(strategy_id: str) -> dict[str, Any] | None:
    with closing(get_db()) as conn:
        row = conn.execute(
            "SELECT * FROM strategy_versions WHERE strategy_id = ? AND status = 'active' LIMIT 1",
            (strategy_id,),
        ).fetchone()
    return _version_row_dict(row) if row else None


def instance_strategy_code_hash(instance: dict[str, Any]) -> str:
    strategy_path = resolve_path(instance.get("strategy_path"))
    if strategy_path and strategy_path.exists():
        try:
            return sha256_file(strategy_path)[:16]
        except Exception:  # noqa: BLE001
            return ""
    return ""


def instance_config_snapshot(instance: dict[str, Any]) -> dict[str, Any]:
    """Lightweight config fingerprint for the version record (not the full file)."""
    snap = {k: instance.get(k) for k in ("strategy_path", "config_path", "pair_universe", "stake_currency") if instance.get(k)}
    config_path = resolve_path(instance.get("config_path"))
    if config_path and config_path.exists():
        try:
            snap["config_hash"] = sha256_file(config_path)[:16]
        except Exception:  # noqa: BLE001
            pass
    return snap


def _earliest_team_trade_open(team_id: str) -> str | None:
    with closing(get_db()) as conn:
        row = conn.execute(
            "SELECT MIN(open_date) AS first_open FROM team_trades WHERE team_id = ? AND open_date IS NOT NULL",
            (team_id,),
        ).fetchone()
    return row["first_open"] if row and row["first_open"] else None


def ensure_baseline_versions() -> None:
    """Give every major-league runtime a v1 'launch' version if it has none, so
    attribution is well-defined and 'Deploy v2' always has a v1 to retire."""
    now = iso_now()
    season = current_league_season().get("season_number")
    for instance in list_instances():
        team_id = str(instance.get("id") or "")
        if not team_id:
            continue
        if strategy_versions_for(team_id):
            continue
        deployed_at = _earliest_team_trade_open(team_id) or current_league_season().get("started_at") or now
        with closing(get_db()) as conn:
            conn.execute(
                """
                INSERT INTO strategy_versions (
                    strategy_id, strategy_family, display_name, version_number, version_label,
                    parent_version_id, status, change_type, change_summary, reason_for_change,
                    evidence_snapshot_json, verdict, code_hash, config_snapshot_json, created_by,
                    season_number, notes, created_at, deployed_at, retired_at
                ) VALUES (?, ?, ?, 1, 'v1', NULL, 'active', 'launch', ?, '', '{}', '', ?, ?, 'system', ?, '', ?, ?, NULL)
                """,
                (
                    team_id,
                    str(instance.get("strategy_family") or ""),
                    str(instance.get("display_name") or instance.get("name") or team_id),
                    "Launch era — the original strategy implementation for this runtime.",
                    instance_strategy_code_hash(instance),
                    json.dumps(instance_config_snapshot(instance)),
                    season,
                    now,
                    deployed_at,
                ),
...
def create_strategy_version(
    ...
    """Insert the next version as a draft, parented to the current active version.
    Governance: change_summary + reason are required (validated by the caller/route)."""
    instance = next((i for i in list_instances() if str(i.get("id")) == strategy_id), {})
    parent = active_strategy_version(strategy_id)
    existing = strategy_versions_for(strategy_id)
    next_number = (max((int(v["version_number"]) for v in existing), default=0) + 1) if existing else 1
    now = iso_now()
    with closing(get_db()) as conn:
        cur = conn.execute(
            """
            ...
            (
                strategy_id,
                str(instance.get("strategy_family") or (parent or {}).get("strategy_family") or ""),
                str(instance.get("display_name") or (parent or {}).get("display_name") or strategy_id),
                next_number,
                f"v{next_number}",
                (parent or {}).get("id"),
                change_type,
                ...
                json.dumps(instance_config_snapshot(instance)),
                created_by,
                current_league_season().get("season_number"),
                now,
            ),
        )
        conn.commit()
        version_id = int(cur.lastrowid)
    return strategy_version_by_id(version_id) or {}


def deploy_strategy_version(version_id: int) -> dict[str, Any]:
    """Make a draft version the active one: retire the prior active, stamp the
    deploy timestamp, write the deploy event, re-attribute trades, and record the
    Chronicle 'Patch Day' chapter. The boundary is the deploy timestamp."""
    version = strategy_version_by_id(version_id)
    if not version:
        raise ValueError("version not found")
    strategy_id = str(version["strategy_id"])
    prior = active_strategy_version(strategy_id)
    now = iso_now()
    with closing(get_db()) as conn:
        if prior and int(prior["id"]) != int(version_id):
            conn.execute(
                "UPDATE strategy_versions SET status = 'retired', retired_at = ? WHERE id = ?",
                (now, int(prior["id"])),
            )
        conn.execute(
            "UPDATE strategy_versions SET status = 'active', deployed_at = COALESCE(deployed_at, ?) WHERE id = ?",
            (now, int(version_id)),
        )
        conn.execute(
            """
            INSERT INTO strategy_version_events (
                event_type, strategy_id, strategy_family, old_version_id, new_version_id,
                season_number, reason, summary, evidence_snapshot_json, created_at
            ) VALUES ('strategy_version_deployed', ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                strategy_id,
                str(version.get("strategy_family") or ""),
                (prior or {}).get("id"),
                int(version_id),
                current_league_season().get("season_number"),
                str(version.get("reason_for_change") or ""),
                str(version.get("change_summary") or ""),
                json.dumps(version.get("evidence_snapshot") or {}),
                now,
            ),
        )
        conn.commit()
    attribute_trade_versions(strategy_id)
    try:
        write_chronicle_day(local_now().date())
    except Exception as exc:  # noqa: BLE001
        log_maintenance("versioning", "warning", f"Chronicle re-write after deploy failed: {exc}")
    return strategy_version_by_id(version_id) or {}


def retire_strategy_version(version_id: int) -> None:
    with closing(get_db()) as conn:
        conn.execute(
            "UPDATE strategy_versions SET status = 'retired', retired_at = COALESCE(retired_at, ?) WHERE id = ?",
            (iso_now(), int(version_id)),
        )
        conn.commit()


def add_version_note(version_id: int, note: str) -> None:
    version = strategy_version_by_id(version_id)
    if not version:
        return
    stamp = local_now().strftime("%Y-%m-%d %H:%M")
    existing = str(version.get("notes") or "").strip()
    combined = (existing + "\n" if existing else "") + f"[{stamp}] {note.strip()}"
    with closing(get_db()) as conn:
        conn.execute("UPDATE strategy_versions SET notes = ? WHERE id = ?", (combined, int(version_id)))
        conn.commit()


def mark_version_pathological(version_id: int, evidence: dict[str, Any] | None = None, verdict: str = "") -> None:
    """Flag a version's phenotype as unhealthy (diagnosis + evidence). Does not
    deploy anything — it records the clinical read on an existing version."""
    version = strategy_version_by_id(version_id)
    if not version:
        return
    merged_evidence = dict(version.get("evidence_snapshot") or {})
    if evidence:
        merged_evidence.update(evidence)
    merged_evidence["pathological"] = True
    final_verdict = verdict or str(version.get("verdict") or "") or "Phenotype flagged as pathological."
    with closing(get_db()) as conn:
        conn.execute(
            "UPDATE strategy_versions SET evidence_snapshot_json = ?, verdict = ? WHERE id = ?",
            (json.dumps(merged_evidence), final_verdict, int(version_id)),
        )
        conn.commit()


def _version_dt(value: Any) -> datetime | None:
    """Parse a timestamp and coerce to naive UTC so deploy timestamps (aware,
    from iso_now) and freqtrade trade dates (naive) compare cleanly."""
    dt = _ml_parse_dt(value)
    if dt is None:
        return None
...
def _version_window_for_date(versions: list[dict[str, Any]], when: Any) -> dict[str, Any] | None:
    """Which version's deploy window contains timestamp `when`. Versions are
    ordered by deployed_at; a trade belongs to the latest version deployed at or
    before `when`. Falls back to the earliest version when `when` predates all."""
    dt = _version_dt(when)
    deployed = [(v, _version_dt(v.get("deployed_at"))) for v in versions if v.get("deployed_at")]
    deployed = [(v, d) for v, d in deployed if d is not None]
    deployed.sort(key=lambda pair: pair[1])
    if not deployed:
...
def attribute_trade_versions(strategy_id: str) -> None:
    """Attribute every team_trades row of this runtime to a version era.
    opened_under is write-once (set only when NULL) so closed trades keep their
    open-era version forever; managed_under tracks who ran the exit; the trade is
    transition_managed when those differ (Timmy v1 inventory closed by v2)."""
    versions = strategy_versions_for(strategy_id)
    if not versions:
        return
    by_id = {int(v["id"]): v for v in versions}
    active = active_strategy_version(strategy_id)
    with closing(get_db()) as conn:
        rows = conn.execute(
            "SELECT rowid, is_open, open_date, close_date, strategy_version_id FROM team_trades WHERE team_id = ?",
            (strategy_id,),
        ).fetchall()
        for row in rows:
            opened_under_id = row["strategy_version_id"]
            if opened_under_id is None:
                opened_version = _version_window_for_date(versions, row["open_date"])
                opened_under_id = int(opened_version["id"]) if opened_version else None
            if int(row["is_open"] or 0):
                managed = active or (by_id.get(opened_under_id) if opened_under_id else None)
            else:
                managed = _version_window_for_date(versions, row["close_date"] or row["open_date"])
            managed_id = int(managed["id"]) if managed else opened_under_id
            transition = 1 if (opened_under_id and managed_id and opened_under_id != managed_id) else 0
            conn.execute(
                "UPDATE team_trades SET strategy_version_id = ?, managed_under_version_id = ?, transition_managed = ? WHERE rowid = ?",
                (opened_under_id, managed_id, transition, row["rowid"]),
            )
        conn.commit()


def version_stats(strategy_id: str, version_id: int) -> dict[str, Any]:
    """Per-version trade analytics from team_trades, split by opened_under version."""
    with closing(get_db()) as conn:
        rows = [dict(r) for r in conn.execute(
            "SELECT * FROM team_trades WHERE team_id = ? AND strategy_version_id = ?",
            (strategy_id, int(version_id)),
        ).fetchall()]
    closed = [t for t in rows if not int(t.get("is_open") or 0)]
    open_rows = [t for t in rows if int(t.get("is_open") or 0)]
    roi_list = [parse_float(t.get("profit_pct")) for t in closed]
    durations = [parse_float(t.get("trade_duration_minutes")) for t in closed]
    wins = [t for t in closed if parse_float(t.get("profit_abs")) > 0]
    winners_roi = [parse_float(t.get("profit_pct")) for t in wins]
    ...
    for t in closed:
        exit_hist[str(t.get("exit_reason") or "unknown")] += 1
    native = sum(1 for t in rows if not int(t.get("transition_managed") or 0))
    transition = sum(1 for t in rows if int(t.get("transition_managed") or 0))
    now = utc_now().replace(tzinfo=None)
    stale = 0
    for t in open_rows:
        opened = _version_dt(t.get("open_date"))
        if opened and (now - opened).total_seconds() > 3 * 86400:
            stale += 1
    return {
        "trades": len(rows),
        "closed": len(closed),
        ...
        "unrealized_pnl": unrealized_pnl,
        "win_rate": (len(wins) / len(closed) * 100.0) if closed else 0.0,
        "avg_roi": stat_mean(roi_list),
        "median_roi": stat_median(roi_list),
        "avg_hold_minutes": stat_mean(durations),
        "median_hold_minutes": stat_median(durations),
        "max_hold_minutes": max(durations, default=0.0),
        "hold_dispersion": stat_pstdev(durations),
        "exit_efficiency": stat_mean(winners_roi),
        "exit_histogram": dict(exit_hist),
        "native_trades": native,
        "transition_managed_trades": transition,
        "stale_inventory": stale,
    }


def version_drift_for(instance: dict[str, Any]) -> dict[str, Any] | None:
    """Detect a strategy-file change with no matching deployed version (drift).
    Returns a descriptor for the Versions banner, or None when in sync."""
    active = active_strategy_version(str(instance.get("id") or ""))
    if not active:
        return None
    current_hash = instance_strategy_code_hash(instance)
    recorded_hash = str(active.get("code_hash") or "")
    if current_hash and recorded_hash and current_hash != recorded_hash:
        return {
            "active_label": active.get("version_label"),
            "active_hash": recorded_hash,
            "current_hash": current_hash,
        }
    return None


def version_power_read(strategy_id: str) -> dict[str, Any]:
    """Tell the Power Rankings reader what basis the score sits on: a full-season
    blend, a single active version, or a fresh transition that hasn't proven out."""
    versions = strategy_versions_for(strategy_id)
    active = next((v for v in versions if v.get("status") == "active"), None)
    if not versions or not active:
        return {"version_basis": "full season record", "version_note": ""}
    season_start = _version_dt(current_league_season().get("started_at"))
    deployed = _version_dt(active.get("deployed_at"))
    retired = [v for v in versions if v.get("status") == "retired"]
    recent_transition = bool(retired) and deployed is not None and season_start is not None and deployed >= season_start
    if recent_transition:
        retired_path = next((v for v in retired if (v.get("evidence_snapshot") or {}).get("pathological")), None)
        note = f"Version transition: {active['version_label']} is newly deployed and under evaluation"
        if retired_path:
            note = (f"Version transition: the full-season record remains weighed down by "
                    f"{retired_path['version_label']} ({retired_path.get('verdict') or 'flagged phenotype'}), "
                    f"but {active['version_label']} is now under evaluation.")
        return {"version_basis": "blended version history", "version_note": note,
                "active_version": active["version_label"]}
    basis = "active version only" if len(versions) == 1 else "blended version history"
    return {"version_basis": basis, "version_note": "", "active_version": active["version_label"]}


def _version_runtime_duration(version: dict[str, Any]) -> str:
    start = _version_dt(version.get("deployed_at"))
    if not start:
        return "—"
    end = _version_dt(version.get("retired_at")) or utc_now().replace(tzinfo=None)
...
def versions_context() -> dict[str, Any]:
    """Per-strategy (runtime) version histories with stats, drift, and the deploy log.
    Standings stay cumulative by strategy; this is the analytics split by version."""
    strategies: list[dict[str, Any]] = []
    for instance in list_instances():
        team_id = str(instance.get("id") or "")
        versions = strategy_versions_for(team_id)
        if not versions:
            continue
        for v in versions:
            v["stats"] = version_stats(team_id, int(v["id"]))
            v["runtime_duration"] = _version_runtime_duration(v)
        active = next((v for v in versions if v.get("status") == "active"), None)
        strategies.append({
            "team_id": team_id,
            "display_name": instance.get("display_name") or instance.get("name") or team_id,
            "strategy_family": instance.get("strategy_family") or "",
            "pair_universe": instance.get("pair_universe") or "",
            "active": active,
            "versions": versions,
            "drift": version_drift_for(instance),
        })
    with closing(get_db()) as conn:
        events = [dict(r) for r in conn.execute(
            "SELECT * FROM strategy_version_events ORDER BY id DESC LIMIT 50"
        ).fetchall()]
    return {
        "strategies": strategies,
        "events": events,
        "change_types": VERSION_CHANGE_TYPES,
        "fingerprints": version_registry(),
    }


# ---------------------------------------------------------------------------
# Competition Environment versioning (the SECOND axis).
#
# Strategy versions track the organism; environment versions track the
# measurement conditions (exchange / stake / universe construction / pairlist
# source). Same per-runtime scoping and the same window-by-deploy-timestamp
# attribution as strategy versions, so a trade carries BOTH a strategy_version
# and an environment_version and we can separate "Timmy changed" from "the
# universe changed".
# ---------------------------------------------------------------------------

ENVIRONMENT_CHANGE_TYPES = {
    "baseline": "Baseline (recorded as-found)",
    "universe_standardization": "Universe standardization (ATL RemotePairList)",
    "pairlist_change": "Pair list change",
    "exchange_change": "Exchange change",
    "stake_change": "Stake currency change",
    "other": "Other environment change",
}

# How a freqtrade pairlist method maps onto a universe-construction label.
_PAIRLIST_CONSTRUCTION = {
    "MarketCapPairList": ("dynamic-marketcap", "freqtrade-marketcap"),
    "RemotePairList": ("atl-remote", "atl-remote"),
    "StaticPairList": ("static-snapshot", "static-config"),
    "VolumePairList": ("dynamic-volume", "freqtrade-volume"),
}


def _environment_row_dict(row: Any) -> dict[str, Any]:
    item = dict(row)
    item["evidence_snapshot"] = decode_jsonish_payload(str(item.get("evidence_snapshot_json") or "{}"), {})
    item["config_snapshot"] = decode_jsonish_payload(str(item.get("config_snapshot_json") or "{}"), {})
    item["change_type_label"] = ENVIRONMENT_CHANGE_TYPES.get(
        str(item.get("change_type") or ""), str(item.get("change_type") or "").replace("_", " ").title())
    return item


def environment_fingerprint(instance: dict[str, Any]) -> dict[str, Any]:
    """Read a runtime's live config and describe the measurement environment it
    is actually trading under (NOT what the label claims)."""
    cfg = load_json(resolve_path(instance.get("config_path")), {}) if instance.get("config_path") else {}
    if not isinstance(cfg, dict):
        cfg = {}
    exchange = str((cfg.get("exchange") or {}).get("name") or "")
    stake = str(cfg.get("stake_currency") or "")
    pls = cfg.get("pairlists") or []
    first = pls[0] if pls else {}
    method = str(first.get("method") or "")
    construction, source = _PAIRLIST_CONSTRUCTION.get(method, (method.lower() or "unknown", method or "unknown"))
    pairlist_name = ""
    if method == "RemotePairList":
        url = str(first.get("pairlist_url") or "")
        pairlist_name = url.rsplit("/", 1)[-1].replace(".json", "") if url else ""
    number_assets = first.get("number_assets")
    if number_assets is None and method == "StaticPairList":
        number_assets = len((cfg.get("exchange") or {}).get("pair_whitelist") or []) or None
    return {
        "exchange": exchange,
        "stake_currency": stake,
        "universe_label": str(instance.get("pair_universe") or ""),
        "universe_construction": construction,
        "pairlist_source": source,
        "pairlist_name": pairlist_name,
        "number_assets": int(number_assets) if number_assets else None,
        "pairlist_method": method,
    }


def _environment_signature(fp: dict[str, Any]) -> str:
    return "|".join([
        str(fp.get("exchange") or ""), str(fp.get("stake_currency") or ""),
        str(fp.get("universe_construction") or ""), str(fp.get("pairlist_source") or ""),
        str(fp.get("pairlist_name") or ""), str(fp.get("number_assets") or ""),
    ])


def environment_versions_for(strategy_id: str) -> list[dict[str, Any]]:
    with closing(get_db()) as conn:
        rows = conn.execute(
            "SELECT * FROM environment_versions WHERE strategy_id = ? ORDER BY version_number ASC",
            (strategy_id,),
        ).fetchall()
    return [_environment_row_dict(row) for row in rows]


def environment_version_by_id(version_id: int) -> dict[str, Any] | None:
    with closing(get_db()) as conn:
        row = conn.execute("SELECT * FROM environment_versions WHERE id = ?", (int(version_id),)).fetchone()
    return _environment_row_dict(row) if row else None


def active_environment_version(strategy_id: str) -> dict[str, Any] | None:
    with closing(get_db()) as conn:
        row = conn.execute(
            "SELECT * FROM environment_versions WHERE strategy_id = ? AND status = 'active' LIMIT 1",
            (strategy_id,),
        ).fetchone()
    return _environment_row_dict(row) if row else None


def _insert_environment_version(strategy_id: str, instance: dict[str, Any], fp: dict[str, Any],
                                number: int, label: str, status: str, change_type: str,
                                change_summary: str, reason: str, evidence: dict[str, Any],
                                created_by: str, parent_id: int | None, deployed_at: str | None) -> int:
    now = iso_now()
    with closing(get_db()) as conn:
        cur = conn.execute(
            """
            INSERT INTO environment_versions (
                strategy_id, display_name, version_number, version_label, parent_version_id, status,
                exchange, stake_currency, universe_label, universe_construction, pairlist_source,
                pairlist_name, number_assets, change_type, change_summary, reason_for_change,
                evidence_snapshot_json, config_snapshot_json, created_by, season_number, notes,
                created_at, deployed_at, retired_at
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, '', ?, ?, NULL)
            """,
            (
                strategy_id, str(instance.get("display_name") or instance.get("name") or strategy_id),
                number, label, parent_id, status,
                fp.get("exchange") or "", fp.get("stake_currency") or "", fp.get("universe_label") or "",
                fp.get("universe_construction") or "", fp.get("pairlist_source") or "",
                fp.get("pairlist_name") or "", fp.get("number_assets"),
                change_type, change_summary, reason, json.dumps(evidence or {}), json.dumps(fp),
                created_by, current_league_season().get("season_number"), now, deployed_at,
            ),
        )
        conn.commit()
...
def ensure_baseline_environments() -> None:
    """Record each major-league runtime's CURRENT environment as env-v1 (the honest
    snapshot of today's setup), so the as-found inconsistency is on the record and
    attribution is well-defined before any standardization deploy."""
    for instance in list_instances():
        team_id = str(instance.get("id") or "")
        if not team_id or environment_versions_for(team_id):
            continue
        fp = environment_fingerprint(instance)
        deployed_at = _earliest_team_trade_open(team_id) or current_league_season().get("started_at") or iso_now()
        vid = _insert_environment_version(
            team_id, instance, fp, number=1, label="env-v1", status="active",
            change_type="baseline",
            change_summary="Environment recorded as-found at the introduction of environment versioning.",
            reason="Baseline snapshot of the runtime's exchange, stake currency and universe construction.",
            evidence={"as_found": fp}, created_by="system", parent_id=None, deployed_at=deployed_at,
        )
        attribute_trade_environments(team_id)
        _ = vid


def create_environment_version(strategy_id: str, fp: dict[str, Any], change_type: str,
                               change_summary: str, reason: str, evidence: dict[str, Any] | None = None,
                               created_by: str = "human") -> dict[str, Any]:
    instance = next((i for i in list_instances() if str(i.get("id")) == strategy_id), {})
    parent = active_environment_version(strategy_id)
    existing = environment_versions_for(strategy_id)
    next_number = (max((int(v["version_number"]) for v in existing), default=0) + 1) if existing else 1
    vid = _insert_environment_version(
        strategy_id, instance, fp, number=next_number, label=f"env-v{next_number}", status="draft",
        change_type=change_type, change_summary=change_summary, reason=reason,
        evidence=evidence or {}, created_by=created_by,
        parent_id=(parent or {}).get("id"), deployed_at=None,
    )
    return environment_version_by_id(vid) or {}


def deploy_environment_version(version_id: int) -> dict[str, Any]:
    version = environment_version_by_id(version_id)
    if not version:
        raise ValueError("environment version not found")
    strategy_id = str(version["strategy_id"])
    prior = active_environment_version(strategy_id)
    now = iso_now()
    with closing(get_db()) as conn:
        if prior and int(prior["id"]) != int(version_id):
            conn.execute("UPDATE environment_versions SET status='retired', retired_at=? WHERE id=?", (now, int(prior["id"])))
        conn.execute("UPDATE environment_versions SET status='active', deployed_at=COALESCE(deployed_at, ?) WHERE id=?", (now, int(version_id)))
        conn.execute(
            """
            INSERT INTO environment_version_events (
                event_type, strategy_id, old_version_id, new_version_id, season_number,
                reason, summary, evidence_snapshot_json, created_at
            ) VALUES ('environment_version_deployed', ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (strategy_id, (prior or {}).get("id"), int(version_id),
             current_league_season().get("season_number"),
             str(version.get("reason_for_change") or ""), str(version.get("change_summary") or ""),
             json.dumps(version.get("evidence_snapshot") or {}), now),
        )
        conn.commit()
    attribute_trade_environments(strategy_id)
    try:
        write_chronicle_day(local_now().date())
    except Exception as exc:  # noqa: BLE001
        log_maintenance("versioning", "warning", f"Chronicle re-write after env deploy failed: {exc}")
    return environment_version_by_id(version_id) or {}


def attribute_trade_environments(strategy_id: str) -> None:
    """Attribute each team_trades row to the environment era live when it OPENED
    (write-once); managed_under tracks the environment that ran the exit."""
    versions = environment_versions_for(strategy_id)
    if not versions:
        return
    active = active_environment_version(strategy_id)
    with closing(get_db()) as conn:
        rows = conn.execute(
            "SELECT rowid, is_open, open_date, close_date, environment_version_id FROM team_trades WHERE team_id = ?",
            (strategy_id,),
        ).fetchall()
        for row in rows:
            opened_id = row["environment_version_id"]
            if opened_id is None:
                ov = _version_window_for_date(versions, row["open_date"])
                opened_id = int(ov["id"]) if ov else None
            if int(row["is_open"] or 0):
                managed = active
            else:
                managed = _version_window_for_date(versions, row["close_date"] or row["open_date"])
            managed_id = int(managed["id"]) if managed else opened_id
            conn.execute(
                "UPDATE team_trades SET environment_version_id = ?, managed_under_environment_version_id = ? WHERE rowid = ?",
                (opened_id, managed_id, row["rowid"]),
            )
        conn.commit()


def environment_drift_for(instance: dict[str, Any]) -> dict[str, Any] | None:
    """Live config environment differs from the active recorded environment version."""
    active = active_environment_version(str(instance.get("id") or ""))
    if not active:
        return None
    fp = environment_fingerprint(instance)
    current_sig = _environment_signature(fp)
    recorded_sig = _environment_signature(active.get("config_snapshot") or active)
    if current_sig != recorded_sig:
        return {"active_label": active.get("version_label"), "current": fp,
                "recorded": active.get("config_snapshot") or {}}
    return None


def planned_atl_remote_payload(instance: dict[str, Any]) -> dict[str, Any]:
    """The Environment v2 target for the standardization migration: same exchange &
    stake, but the universe now comes from the single league-controlled ATL
    RemotePairList source instead of a frozen snapshot or freqtrade's built-in list.

    The served manifest id is `{universe}_{exchange}_{markettype}_{quote}` (see
    manifest_name); pointing a config at the bare universe name returns an empty
    pairlist. `manifest_available` is False when no manifest exists for the runtime's
    exchange (e.g. hyperliquid) — such a runtime must NOT be migrated."""
    label = str(instance.get("pair_universe") or "")
    digits = "".join(ch for ch in label if ch.isdigit())
    size = int(digits) if digits else 50
    universe = "top20_marketcap" if size <= 20 else "top50_marketcap" if size <= 50 else "top100_marketcap"
    current = environment_fingerprint(instance)
    exchange = str(current.get("exchange") or "")
    stake = str(current.get("stake_currency") or "USDT")
    market_type = "futures"
    manifest_id = manifest_name(universe, exchange, market_type, stake)
    manifest = get_pairlist_manifest(manifest_id)
    return {
        "exchange": exchange,
        "stake_currency": stake,
        "universe_label": label,
        "universe": universe,
        "universe_construction": "atl-remote",
        "pairlist_source": "atl-remote",
        "pairlist_name": manifest_id,
        "manifest_available": manifest is not None,
        "manifest_pair_count": len((manifest or {}).get("pairs", [])),
        "number_assets": size,
        "from": current,
    }


def migrate_runtime_to_atl_remote(team_id: str, fire: bool = False) -> dict[str, Any]:
    """Stage (fire=False) or perform (fire=True) the ATL RemotePairList migration for
    one runtime: rewrite its config's pairlist to the league manifest (exchange
    unchanged), then create + deploy Environment v2. The GATE: callers preview with
    fire=False and only fire on an explicit operator action."""
    instance = next((i for i in list_instances() if str(i.get("id")) == team_id), None)
    if not instance:
        return {"ok": False, "error": "unknown runtime"}
    payload = planned_atl_remote_payload(instance)
    if not fire:
        return {"ok": True, "preview": payload}
    # Guard: never repoint a runtime at a manifest that doesn't exist (e.g. no
    # hyperliquid manifest) — that yields an empty pairlist and halts trading.
    if not payload.get("manifest_available"):
        return {"ok": False, "error": f"no ATL manifest for {payload['pairlist_name']} (exchange not covered)",
                "preview": payload}
    config_path = resolve_path(instance.get("config_path"))
    if not config_path or not config_path.exists():
    ...
    cfg["pairlists"] = [{
        "method": "RemotePairList",
        "pairlist_url": f"{pairlist_manifest_base_url()}/pairlists/{payload['pairlist_name']}.json",
        "number_assets": int(payload["number_assets"]),
        "refresh_period": int(refresh),
        "keep_pairlist_on_failure": True,
    }]
    config_path.write_text(json.dumps(cfg, indent=2), encoding="utf-8")
    fp = environment_fingerprint(instance)  # re-read post-rewrite (now atl-remote)
    version = create_environment_version(
        team_id, fp, change_type="universe_standardization",
        change_summary="Migrated to ATL RemotePairList — universe now drawn from the single league-controlled source.",
        reason=("Major-league runtimes were operating on a mix of dynamic market-cap and static snapshots "
                "across different exchanges; standardizing the universe source improves experimental consistency."),
        evidence={"from": payload.get("from"), "to": fp}, created_by="human",
    )
    deploy_environment_version(int(version["id"]))
    return {"ok": True, "deployed": environment_version_by_id(int(version["id"]))}


def migrate_major_league_to_atl_remote(fire: bool = False) -> dict[str, Any]:
    """League-wide gate: preview or perform the standardization across every
    major-league runtime as one infrastructure event."""
    results = []
    for instance in list_instances():
        results.append({"team_id": instance.get("id"),
                        "display_name": instance.get("display_name"),
                        **migrate_runtime_to_atl_remote(str(instance.get("id") or ""), fire=fire)})
    return {"ok": True, "fired": fire, "runtimes": results}


def environment_version_stats(strategy_id: str, version_id: int) -> dict[str, Any]:
    """Per-environment-era trade analytics (trades opened under this environment)."""
    with closing(get_db()) as conn:
        rows = [dict(r) for r in conn.execute(
            "SELECT * FROM team_trades WHERE team_id = ? AND environment_version_id = ?",
            (strategy_id, int(version_id)),
        ).fetchall()]
    closed = [t for t in rows if not int(t.get("is_open") or 0)]
    roi_list = [parse_float(t.get("profit_pct")) for t in closed]
...
def environments_context() -> dict[str, Any]:
    """Per-runtime environment histories + the staged migration preview for /versions."""
    runtimes = []
    for instance in list_instances():
        team_id = str(instance.get("id") or "")
        versions = environment_versions_for(team_id)
        if not versions:
            continue
        for v in versions:
            v["stats"] = environment_version_stats(team_id, int(v["id"]))
            v["runtime_duration"] = _version_runtime_duration(v)
        active = next((v for v in versions if v.get("status") == "active"), None)
        runtimes.append({
            "team_id": team_id,
            "display_name": instance.get("display_name") or team_id,
            "active": active,
            "versions": versions,
            "drift": environment_drift_for(instance),
            "migration_preview": planned_atl_remote_payload(instance),
            "needs_migration": bool(active and active.get("universe_construction") != "atl-remote"),
        })
    with closing(get_db()) as conn:
        events = [dict(r) for r in conn.execute(
            "SELECT * FROM environment_version_events ORDER BY id DESC LIMIT 50").fetchall()]
    return {"runtimes": runtimes, "events": events,
            "any_needs_migration": any(r["needs_migration"] for r in runtimes)}
...
def build_season_awards(preview_rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Season-preview projection: the SAME award decisions that will be granted at turnover,
    so the preview matches reality (previously hardcoded to only 4)."""
    results = major_league_award_results(preview_rows)
    return [
        {"title": r["title"], "emoji": r["emoji"], "dimension": r["dimension"],
...
def generate_quarterly_report(quarter_number: int, force: bool = False) -> dict[str, Any] | None:
    ...
    if get_setting("ollama_api_key", "") and teams:
        try:
            ...
            log_maintenance("quarterly", "degraded", f"Quarterly LLM call failed, using deterministic text: {exc}")
    for t in teams:
...
def major_league_award_results(rows: list[dict[str, Any]], season_number: int = 0) -> list[dict[str, Any]]:
    """Pure award SELECTION over a set of team rows (stored season reviews OR live preview
    rows — they carry the same fields). Returns the winner of each major-league award. Shared
    by the turnover grant path AND the season-preview projection so they never diverge."""
    out: list[dict[str, Any]] = []
    if not rows:
        return out

    def add(award_type: str, row: dict[str, Any] | None, reason: str, value: float) -> None:
        if not row:
            return
        meta = STRATEGY_AWARD_CATALOG.get(award_type, {})
        out.append({"award_type": award_type, "title": meta.get("title", award_type),
                    "emoji": meta.get("emoji", "🏆"), "dimension": meta.get("dimension", ""),
                    "winner_team_id": str(row.get("team_id")), "winner": str(row.get("team_name") or row.get("team_id")),
                    "reason": reason, "value": float(value or 0.0)})

    champ = max(rows, key=lambda r: parse_float(r.get("total_pnl")))
    add("champion", champ, f"Finished #1 with {parse_float(champ.get('total_pnl')):+.2f} total P&L.", parse_float(champ.get("total_pnl")))

    surg_pool = [r for r in rows if int(r.get("closed_trades") or 0) >= 3]
    if surg_pool:
        surg = max(surg_pool, key=lambda r: parse_float(r.get("avg_roi")))
        add("surgeon", surg, f"{parse_float(surg.get('avg_roi')):.2f}% average ROI across {int(surg.get('closed_trades') or 0)} trades.", parse_float(surg.get("avg_roi")))
    ...
    if parse_float(iron.get("runtime_hours")) > 0:
        add("iron_bot", iron, f"{parse_float(iron.get('runtime_hours')):.1f} runtime hours at {parse_float(iron.get('heartbeat_ratio')) * 100:.0f}% heartbeat.", parse_float(iron.get("runtime_hours")))

    gwell = max(rows, key=lambda r: int(r.get("closed_trades") or 0))
    if int(gwell.get("closed_trades") or 0) > 0:
        add("gravity_well", gwell, f"Pulled in {int(gwell.get('closed_trades') or 0)} closed trades — the league's highest throughput.", int(gwell.get("closed_trades") or 0))
    ...
    if steady_pool:
        ...
        add("steady_hand", steady, f"Held max drawdown to {parse_float(steady.get('max_drawdown')):.1f}% while finishing profitable.", parse_float(steady.get("max_drawdown")))
    ...
    if len(by_pnl) >= 2:
        ...
        if top > 0 and (top - second) >= 0.25 * (abs(second) + 1.0):
            add("dragon", by_pnl[0], f"Dominated the season — beat the runner-up by {top - second:+.2f} P&L.", top - second)
    ...
    if surv_pool:
        ...
        add("survivor", surv, f"Clawed back to {parse_float(surv.get('total_pnl')):+.2f} after a {parse_float(surv.get('max_drawdown')):.1f}% drawdown.", parse_float(surv.get("max_drawdown")))
    ...
    add("wildcard", wild_row, "A true specialist — elite in one dimension, deliberately weak in another.", wild_disp if wild_row else 0.0)
    ...
    for r in top_half:
        if not team_has_prior_season_review(str(r.get("team_id")), season_number):
            add("rookie", r, f"A standout debut — finished top-half at {parse_float(r.get('total_pnl')):+.2f} P&L.", parse_float(r.get("total_pnl")))
            break
    return out
...
def evaluate_major_league_season_awards(season: dict[str, Any], reviews: list[dict[str, Any]]) -> None:
    ...
    for award in major_league_award_results(rows, season_number):
        grant_strategy_award(award["award_type"], season_number, season_label, "team",
                             award["winner_team_id"], award["winner"], award["reason"], award["value"])
...
def ml_biology_context() -> dict[str, Any]:
    ...
    return {
        ...
        "version_breakdowns": ml_version_breakdowns(),
    }
...
# Maintenance noise that is NOT a real incident: the system gracefully degraded (LLM outage →
# deterministic fallback) or hit a known-transient provider error. These must not make the
# Chronicle declare a "Crisis Day".
_MAINTENANCE_ROUTINE_MESSAGE_HINTS = (
    "using deterministic", "llm call failed", "llm narrative failed", "circuit open",
    "succeeded with fallback model", "load_markets failed for hyperliquid",
)


def _maintenance_is_routine(maintenance_type: str, status: str, message: str) -> bool:
    s = str(status or "").lower()
    if s in {"degraded", "fallback", "skipped", "info", "ok", "success"}:
        return True
    msg = str(message or "").lower()
    return any(hint in msg for hint in _MAINTENANCE_ROUTINE_MESSAGE_HINTS)
...
def llm_circuit_open() -> bool:
    """True when the LLM has failed enough consecutive times recently that we should skip
    calling it and go straight to deterministic fallbacks — avoids multi-minute hangs and
    log spam while the provider is down. Resets automatically after the cooldown."""
    fails = int(parse_float(get_setting("ollama_consecutive_failures", "0")))
    threshold = max(1, int(parse_float(get_setting("ollama_breaker_threshold", "4"))))
    if fails < threshold:
        return False
    last_fail = resolve_optional_datetime(get_setting("ollama_last_failure_at", ""))
    if not last_fail:
        return False
    cooldown_min = max(1.0, parse_float(get_setting("ollama_breaker_cooldown_minutes", "15")) or 15.0)
    return (utc_now() - normalize_utc(last_fail)).total_seconds() < cooldown_min * 60.0


def _llm_record_success() -> None:
    set_setting("ollama_consecutive_failures", "0")
    set_setting("ollama_last_success_at", iso_now())


def _llm_record_failure() -> None:
    fails = int(parse_float(get_setting("ollama_consecutive_failures", "0"))) + 1
    set_setting("ollama_consecutive_failures", str(fails))
    set_setting("ollama_last_failure_at", iso_now())


def llm_health() -> dict[str, Any]:
    """LLM resilience status for /settings/ai."""
    fails = int(parse_float(get_setting("ollama_consecutive_failures", "0")))
    threshold = max(1, int(parse_float(get_setting("ollama_breaker_threshold", "4"))))
    return {
        ...
        "last_success_at": get_setting("ollama_last_success_at", ""),
        "last_failure_at": get_setting("ollama_last_failure_at", ""),
        "cooldown_minutes": int(parse_float(get_setting("ollama_breaker_cooldown_minutes", "15")) or 15),
    }
...
def ollama_chat(
    ...
    # Circuit breaker: when the provider has been failing, skip the call entirely so callers
    # fall back to deterministic output instantly instead of hanging through every retry.
    if llm_circuit_open():
        raise RuntimeError("Ollama circuit open (recent consecutive failures); using deterministic fallback.")
...
def build_strategy_timeline(closed: list[dict[str, Any]], max_days: int = 120) -> dict[str, Any]:
    """Shared per-day PnL/trade-count timeline + version bands + forced-aware summary for BOTH
    leagues. Each trade dict carries close_date, profit_abs, profit_ratio, exit_reason,
    forced(0/1), is_short, and tl_version (the version-band key; relabeled v1,v2,... by order)."""
    ...
    for trade in closed:
        ...
        if not dt:
            continue
        day = dt.date()
        bucket = daily.setdefault(day, {"strategy_pnl": 0.0, "forced_pnl": 0.0, "count": 0})
        ...
        bucket["count"] += 1
        version = str(trade.get("tl_version") or "")
        if version:
    ...
    summary = {
        ...
        "realized_pnl": round(strat_pnl + forced_pnl, 4), "strategy_pnl": round(strat_pnl, 4),
        "forced_pnl": round(forced_pnl, 4), "forced_exits": forced_count,
        "autonomous_share": round(comp["autonomous_share"] * 100.0, 0),
        "stop_share": round(comp["stop_share"] * 100.0, 0),
        "forced_share": round(comp["forced_share"] * 100.0, 0),
        "autonomous_winners": comp["autonomous_winners"],
        "exit_autonomy_failed": exit_autonomy_failed(comp),
        "version_count": len(version_span), "active_days": len(daily),
    }
    ...
    bands = [
        {"label": version_label[ver], "version": ver, "color": version_color[ver],
         "start": span[0].isoformat(), "end": span[1].isoformat(), "days": (span[1] - span[0]).days + 1}
    ...
    svg = _strategy_timeline_svg(all_days, daily, ordered_versions, version_label, version_color)
...
def dev_strategy_timeline(slug: str, max_days: int = 120) -> dict[str, Any]:
    """All-time per-day timeline for a dev strategy from dev_all_time_trades (survives the
    per-shift wipe). PnL is strategy-chosen exits; bell force-exits are tracked separately."""
    closed = [t for t in dev_all_time_trades(slug) if not int(t.get("is_open") or 0)]
    for t in closed:
        t["tl_version"] = str(t.get("strategy_version") or "")
    return build_strategy_timeline(closed, max_days)


def team_strategy_timeline(team_id: str, max_days: int = 120) -> dict[str, Any]:
    """All-time per-day timeline for a major-league runtime from team_trades — the same readable
    component the dev profile uses. Version bands come from the trade's strategy_version era;
    forced = a force_exit reason (rare in continuous major-league play)."""
    version_label_by_id = {int(v["id"]): v["version_label"] for v in strategy_versions_for(team_id)}
    closed = [dict(t) for t in team_trade_rows(team_id) if not int(t["is_open"] or 0)]
    for t in closed:
        t["forced"] = 1 if str(t.get("exit_reason") or "").lower() in DEV_FORCED_EXIT_REASONS else 0
        vid = t.get("strategy_version_id")
        t["tl_version"] = version_label_by_id.get(int(vid)) if vid else ""
    return build_strategy_timeline(closed, max_days)


def _strategy_timeline_svg(all_days, daily, ordered_versions, version_label, version_color) -> str:
    """Readable self-contained inline SVG (no JS): a version ribbon + legend on top; the main
    plot is ONLY the daily strategy-PnL (green up / red down, diverging from zero) so the signal
    is never obscured; a dedicated muted activity lane below shows trade-count per day with amber
    diamonds marking forced (shift-bell) days; date labels at the very bottom."""
    n = len(all_days)
    slot_w = max(10, min(28, int(1000 / max(1, n))))
    left_pad, right_pad = 50, 16
    ribbon_y, ribbon_h = 6, 14
    legend_h = 14
    top_pad = ribbon_y + ribbon_h + legend_h + 6           # ribbon + legend
    plot_h = 120
    lane_gap, lane_h, label_h = 8, 16, 16
    width = left_pad + n * slot_w + right_pad
    plot_top = top_pad
    plot_bottom = plot_top + plot_h
    zero_y = plot_top + plot_h / 2
    half = plot_h / 2 - 6
    lane_top = plot_bottom + lane_gap
    lane_bottom = lane_top + lane_h
    height = lane_bottom + label_h
    ...
    parts: list[str] = [
        ...
        f'style="max-width:100%;height:auto;max-height:260px;font-family:inherit;display:block">',
        '<style>.tl-lbl{fill:#64748b;font-size:10px}.tl-axis{stroke:#cbd5e1;stroke-width:1}'
        '.tl-leg{fill:#475569;font-size:10px}</style>',
    ]

    # Version ribbon (top).
    for ver, span in ordered_versions:
        si = day_index.get(span[0]); ei = day_index.get(span[1])
        ...
        parts.append(
            f'<rect x="{x0:.1f}" y="{ribbon_y}" width="{max(2.0, x1 - x0):.1f}" height="{ribbon_h}" rx="3" '
            f'fill="{color}" opacity="0.30"><title>{version_label[ver]} — {ver}</title></rect>')
        if x1 - x0 > 26:
            parts.append(
                f'<text x="{(x0 + x1) / 2:.1f}" y="{ribbon_y + 11}" text-anchor="middle" '
                f'style="fill:{color};font-size:10px;font-weight:600">{version_label[ver]}</text>')

    # Legend row (left-aligned under the ribbon).
    ly = ribbon_y + ribbon_h + 11
    parts.append(f'<rect x="{left_pad}" y="{ly - 8}" width="9" height="9" rx="1" fill="#22c55e"/>'
                 f'<text x="{left_pad + 13}" y="{ly}" class="tl-leg">profit day</text>')
    parts.append(f'<rect x="{left_pad + 78}" y="{ly - 8}" width="9" height="9" rx="1" fill="#ef4444"/>'
                 f'<text x="{left_pad + 91}" y="{ly}" class="tl-leg">loss day</text>')
    parts.append(f'<rect x="{left_pad + 150}" y="{ly - 8}" width="9" height="9" rx="1" fill="#94a3b8" opacity="0.5"/>'
                 f'<text x="{left_pad + 163}" y="{ly}" class="tl-leg">trade count</text>')
    parts.append(f'<path d="M{left_pad + 244} {ly - 4} l3 -4 l3 4 l-3 4 z" fill="#f59e0b"/>'
                 f'<text x="{left_pad + 253}" y="{ly}" class="tl-leg">forced exit</text>')

    # Zero line + y-axis labels (PnL).
    ...
    # Activity-lane label.
    parts.append(f'<text x="{left_pad - 6}" y="{lane_bottom - 4:.1f}" text-anchor="end" class="tl-lbl">trades</text>')
    parts.append(f'<line x1="{left_pad}" y1="{lane_bottom:.1f}" x2="{width - right_pad}" y2="{lane_bottom:.1f}" class="tl-axis"/>')

    bar_w = max(3.0, slot_w * 0.62)
    label_step = max(1, math.ceil(n / 12))
    for i, day in enumerate(all_days):
        bucket = daily.get(day)
        x = cx(i)
        if bucket:
            # PnL bar diverging from zero (the headline signal, uncluttered).
            pnl = bucket["strategy_pnl"]
            bh = (abs(pnl) / max_abs) * half
            y = (zero_y - bh) if pnl >= 0 else zero_y
            ...
            # Activity lane: trade-count bar (muted) in its own strip — no longer fights the PnL.
            if bucket["count"]:
                ah = (bucket["count"] / max_count) * lane_h
                parts.append(
                    f'<rect x="{x - bar_w / 2:.1f}" y="{lane_bottom - ah:.1f}" width="{bar_w:.1f}" '
                    f'height="{max(0.6, ah):.1f}" fill="#94a3b8" opacity="0.45"/>')
            if bucket["forced_pnl"]:  # amber diamond marks a shift-bell force-exit day
                cyf = lane_top - 3
                parts.append(f'<path d="M{x:.1f} {cyf - 3} l3 3 l-3 3 l-3 -3 z" fill="#f59e0b"><title>'
                             f'{day.isoformat()}: forced {bucket["forced_pnl"]:+.2f}</title></path>')
        if i % label_step == 0:
            parts.append(f'<text x="{x:.1f}" y="{height - 4}" text-anchor="middle" class="tl-lbl">'
...
def ml_collect_trades(strategy: dict[str, Any], version_id: int | None = None) -> list[dict[str, Any]]:
    ...
    When version_id is given (main kind), restrict to that version's open-era
    trades so the lab can read one generation/phenotype at a time."""
    ...
    if kind == "main":
        ...
        query = (
            ...
            "trade_duration_minutes FROM team_trades WHERE team_id = ?"
        )
        params: list[Any] = [team_id]
...
# Compact set of phenotype-revealing telemetry categories for the version overlay.
_ML_VERSION_OVERLAY_CATEGORIES = ("avg_hold_time", "avg_roi", "win_rate", "exit_efficiency", "unrealized_drag", "selectivity")


def _ml_version_telemetry_summary(strategy: dict[str, Any], version_id: int | None) -> dict[str, Any]:
    trades = ml_collect_trades(strategy, version_id=version_id)
    vector = ml_compute_telemetry(strategy, trades)
    summary = {cat: round(float(vector.get(cat, {}).get("value", 0.0)), 3) for cat in _ML_VERSION_OVERLAY_CATEGORIES}
    summary["sample_size"] = vector.get("win_rate", {}).get("sample_size", 0)
    return summary


def ml_version_breakdowns() -> list[dict[str, Any]]:
    """For each main-league organism that has more than one version era, the lab
    reads each generation/phenotype separately plus the all-season blend, so it can
    say 'v1 expressed an exit-pathology phenotype; v2 is a descendant intended to
    preserve entry selectivity' instead of judging the whole organism by one era."""
    out: list[dict[str, Any]] = []
    for strategy in ml_registry_all(active_only=False):
        if str(strategy.get("kind") or "main") != "main":
            continue
        team_id = str(strategy.get("source_team_id") or "")
        if not team_id:
            continue
        ...
        for v in versions:
            version_rows.append({
                ...
                "status": v["status"],
                "change_type_label": v["change_type_label"],
                "verdict": v.get("verdict") or "",
                "pathological": bool((v.get("evidence_snapshot") or {}).get("pathological")),
                "telemetry": _ml_version_telemetry_summary(strategy, int(v["id"])),
            })
        out.append({
            "slug": strategy.get("slug"),
            "name": strategy.get("name"),
            "team_id": team_id,
            "categories": list(_ML_VERSION_OVERLAY_CATEGORIES),
            "versions": version_rows,
            "blended": _ml_version_telemetry_summary(strategy, None),
        })
    return out
...
def ml_compute_telemetry(strategy: dict[str, Any], trades: list[dict[str, Any]]) -> dict[str, dict[str, Any]]:
    ...
    measured = {
        ...
        # Strategy-chosen (non-stop, non-forced) share of closes — see ML_TELEMETRY_CATEGORIES.
        "exit_autonomy": shift_exit_composition(closed)["autonomous_share"] * 100.0,
...
def run_ml_telemetry_cycle() -> None:
    ...
    if get_setting("ollama_api_key", "") and findings:
        try:
            ...
            log_maintenance("ml", "degraded", f"Telemetry essay LLM call failed, using deterministic essay: {exc}")
...
def run_ml_evolution_review() -> None:
    ...
    if get_setting("ollama_api_key", ""):
        try:
            ...
            log_maintenance("ml", "degraded", f"Evolution review LLM narrative failed, using deterministic report: {exc}")
...
CHRONICLE_CLASSES: list[tuple[str, str, str]] = [
    ...
    ("infrastructure", "🏗️", "Infrastructure Day"),
    ("patch", "🩹", "Patch Day"),
...
def _chronicle_collect_events(day: Any) -> dict[str, list[dict[str, str]]]:
    ...
    with closing(get_db()) as conn:
        ...
        # 🔥 crisis — GENUINE maintenance errors/warnings only. Routine graceful degradation
        # (LLM outage → deterministic fallback, known-transient provider errors) is skipped so
        # a quiet day of LLM noise is no longer mislabeled a Crisis Day.
        for row in conn.execute("SELECT maintenance_type, status, message, created_at FROM maintenance_runs WHERE status IN ('error', 'warning') ORDER BY id DESC LIMIT 1000"):
            if on_day(row["created_at"]) and not _maintenance_is_routine(row["maintenance_type"], row["status"], row["message"]):
        ...
        # 🩹 patch — a strategy version was deployed mid-season (the v1->v2 boundary)
        for row in conn.execute(
            "SELECT e.strategy_id, e.summary, e.reason, v.display_name, v.version_label, e.created_at "
            "FROM strategy_version_events e LEFT JOIN strategy_versions v ON v.id = e.new_version_id "
            "WHERE e.event_type = 'strategy_version_deployed' ORDER BY e.id DESC LIMIT 1000"
        ):
            if on_day(row["created_at"]):
                name = str(row["display_name"] or row["strategy_id"])
                label = str(row["version_label"] or "new version")
                detail = f"{name} {label} deployed"
                summary = str(row["summary"] or row["reason"] or "").strip()
                if summary:
                    detail += f" — {summary}"
                events["patch"].append({"kind": "version_deployed", "ref": str(row["strategy_id"]), "detail": detail})
        # 🏗️ infrastructure — a competition-environment version was deployed (e.g. universe migration)
        for row in conn.execute(
            "SELECT e.strategy_id, e.summary, e.reason, v.display_name, v.version_label, v.universe_construction, e.created_at "
            "FROM environment_version_events e LEFT JOIN environment_versions v ON v.id = e.new_version_id "
            "WHERE e.event_type = 'environment_version_deployed' ORDER BY e.id DESC LIMIT 1000"
        ):
            if on_day(row["created_at"]):
                name = str(row["display_name"] or row["strategy_id"])
                label = str(row["version_label"] or "new environment")
                detail = f"{name} {label} deployed"
                summary = str(row["summary"] or row["reason"] or "").strip()
                if summary:
                    detail += f" — {summary}"
                events["infrastructure"].append({"kind": "environment_deployed", "ref": str(row["strategy_id"]), "detail": detail})
...
def _chronicle_fallback_blurb(day: Any, classification: str, events: dict[str, list[dict[str, str]]]) -> tuple[str, str]:
    ...
    if classification == "infrastructure" and primary:
        lead = primary[0]["detail"]
        blurb = (
            f"{lead}. A competition-environment change was recorded: the measurement conditions a runtime "
            "trades under (exchange, stake, or universe construction) moved to a new era, with pre- and "
            "post-change results preserved separately. The game is unchanged — this standardizes the "
            "conditions everyone is measured in so results stay attributable."
        )
        if len(primary) > 1:
            blurb += f" {len(primary) - 1} other environment transition{'s' if len(primary) != 2 else ''} were recorded."
        return f"{pretty} — {label}", blurb
    if classification == "patch" and primary:
        lead = primary[0]["detail"]
        blurb = (
            f"{lead}. A strategy was patched mid-season: the prior version is retired and its era sealed, "
            "while the new generation takes over future entries. The strategy identity is unchanged — this "
            "marks the boundary between one version era and the next, not a new strategy."
        )
        if len(primary) > 1:
            blurb += f" {len(primary) - 1} other version transition{'s' if len(primary) != 2 else ''} were recorded."
        return f"{pretty} — {label}", blurb
...
def write_chronicle_day(day: Any) -> dict[str, Any]:
    ...
    if get_setting("ollama_api_key", "") and event_list:
        try:
            ...
            log_maintenance("chronicle", "degraded", f"Chronicle LLM call failed, using deterministic blurb: {exc}")
...
def maintenance_loop() -> None:
    while True:
        ...
        # Backtest Calibration Lab: pair closed live windows with matching backtests.
        if maintenance_due("backtest_calibration_last_run", "backtest_calibration_interval_minutes"):
            try:
                run_backtest_calibration_cycle()
            except Exception as exc:  # noqa: BLE001
                log_maintenance("background", "error", f"Backtest calibration cycle failed: {exc}")
        if get_setting("ollama_api_key", ""):
...
def schedule_jobs() -> list[dict[str, Any]]:
    ...
    return [
        ...
        {
            "name": "Backtest Calibration Cycle", "category": "Intraday",
            "cadence": f"Every {int(parse_float(get_setting('backtest_calibration_interval_minutes', '360')) or 360)} min",
            "trigger": "maintenance_due; pairs closed live windows (dev shifts + major rolling) with backtests",
            "last_run": _schedule_fmt(get_setting("backtest_calibration_last_run", "")),
            "next_run": "interval" if calibration_enabled() else "disabled",
            "source": "maintenance_loop → run_backtest_calibration_cycle (main.py)",
            "writes_to": "calibration_windows, backtest_jobs (paired_calibration)",
            "uses_llm": no, "status": "Active" if calibration_enabled() else "Disabled",
            "notes": "Measures how faithfully the simulator reproduces dry-run reality (reality score), flags custom-exit-inactive runs as invalid. Measure first, modify second.",
        },
...
def standings_rows() -> list[dict[str, Any]]:
    ...
    for instance in list_instances():
        ...
        # Cumulative totals stay strategy-level; the active version is a small indicator only.
        active = active_strategy_version(instance["id"])
        row["active_version"] = active["version_label"] if active else ""
        row["version_count"] = len(strategy_versions_for(instance["id"]))
        rows.append(row)
...
def trade_explorer_rows(team_filter: str = "", version_filter: str = "") -> list[dict[str, Any]]:
    rows: list[dict[str, Any]] = []
    team_lookup = {item["id"]: item["display_name"] for item in list_instances()}
    # version_label per version id (for the row badge) keyed by team.
    version_labels: dict[int, str] = {}
    env_labels: dict[int, str] = {}
    for instance in list_instances():
        for v in strategy_versions_for(str(instance.get("id") or "")):
            version_labels[int(v["id"])] = v["version_label"]
        for v in environment_versions_for(str(instance.get("id") or "")):
            env_labels[int(v["id"])] = v["version_label"]
    clauses: list[str] = []
    params: list[Any] = []
    if team_filter:
        clauses.append("team_id = ?")
        params.append(team_filter)
    if version_filter and team_filter:
        # Resolve a label like "v2" to this team's version id.
        target = next((int(v["id"]) for v in strategy_versions_for(team_filter)
                       if v["version_label"] == version_filter), None)
    ...
    with closing(get_db()) as conn:
        trades = conn.execute(
            f"SELECT * FROM team_trades{where} ORDER BY COALESCE(close_date, open_date) DESC",
            tuple(params),
        ).fetchall()
    for row in trades:
        keys = row.keys()
        opened_id = row["strategy_version_id"] if "strategy_version_id" in keys else None
        managed_id = row["managed_under_version_id"] if "managed_under_version_id" in keys else None
        env_id = row["environment_version_id"] if "environment_version_id" in keys else None
        is_closed = not int(row["is_open"] or 0)
        managed_label = version_labels.get(int(managed_id)) if managed_id else ""
        rows.append(
            {
                ...
                "version_label": version_labels.get(int(opened_id)) if opened_id else "",
                # managed = who ran the exit; for a closed trade this is the closed_version.
                "managed_label": managed_label,
                "closed_label": managed_label if is_closed else "",
                "transition_managed": bool(row["transition_managed"]) if "transition_managed" in keys else False,
                "exchange": row["exchange_name"] or "",
                "environment_label": env_labels.get(int(env_id)) if env_id else "",
            }
        )
    return rows
...
def startup_event() -> None:
    ...
    ensure_baseline_environments()
    reclaim_orphaned_backtests(force_all=True)  # nothing is running at startup; clear stale locks
    sync_development_pipeline()
...
def team_page(request: Request, team_id: str) -> HTMLResponse:
    ...
    team_versions = strategy_versions_for(team_id)
    for v in team_versions:
        v["stats"] = version_stats(team_id, int(v["id"]))
        v["runtime_duration"] = _version_runtime_duration(v)
    active_version = next((v for v in team_versions if v.get("status") == "active"), None)
    active_environment = active_environment_version(team_id)
    # Parity with the dev profile: a readable all-time trading timeline, per-version telemetry,
    # and the exit-mix card — all from team_trades using the shared components.
    timeline = team_strategy_timeline(team_id)
    version_telemetry = next((b for b in ml_version_breakdowns() if str(b.get("team_id")) == team_id), None)
    exit_comp = shift_exit_composition([dict(t) for t in trades if not int(t["is_open"] or 0)])
    exit_comp["failed"] = exit_autonomy_failed(exit_comp)
    return templates.TemplateResponse(
        request,
        "team.html",
        {
...
def trade_explorer_page(request: Request) -> HTMLResponse:
    team_filter = request.query_params.get("team", "").strip()
    version_filter = request.query_params.get("version", "").strip()
    compare = request.query_params.get("compare", "").strip() in {"1", "true", "yes", "on"}
    team_versions = strategy_versions_for(team_filter) if team_filter else []
    teams = [{"id": i["id"], "display_name": i.get("display_name") or i["id"]} for i in list_instances()]
    ...
    if compare and team_filter and len(team_versions) >= 1:
        # Side-by-side aggregates per version of the selected runtime.
        context["compare_columns"] = [
            {"version": v, "stats": version_stats(team_filter, int(v["id"])),
             "rows": trade_explorer_rows(team_filter, v["version_label"])}
            for v in team_versions
        ]
        context["rows"] = []
    else:
        context["rows"] = trade_explorer_rows(team_filter, version_filter)
    return templates.TemplateResponse(
        request,
        "trade_explorer.html",
        page_context_bundle("trade-explorer", "Trade Explorer", page_title="Trade Explorer", **context),
    )
...
def backtesting_department_context() -> dict[str, Any]:
    ...
    with closing(get_db()) as conn:
        ...
        recent = [dict(row) for row in conn.execute(
            ...
            SELECT j.job_id, j.status, j.strategy_name, j.universe_name, j.completed_at, j.failure_reason,
                   j.run_id, j.priority_bucket, j.job_kind, r.total_pnl, r.win_rate, r.avg_roi, r.closed_trades,
                   r.max_drawdown, r.validity_status, r.validity_reason
    ...
    # Only VALID results count as strategy evidence — invalid (e.g. custom_exit inactive),
    # suspect, failed and calibration_only runs must not pollute the coverage matrix.
    latest: dict[tuple[str, str], dict[str, Any]] = {}
    for row in result_rows:
        if str(row["validity_status"] if "validity_status" in row.keys() else "valid") != "valid":
            continue
...
def backtest_calibration_context() -> dict[str, Any]:
    windows = calibration_windows_recent(60)
    scored = [w for w in windows if w.get("status") == "scored"]
    return {
        "engine_version": BACKTEST_ENGINE_VERSION,
        "engine_summary": engine_calibration_summary(),
        "windows": windows,
        "scored_count": len(scored),
        "pending_count": sum(1 for w in windows if w.get("status") in ("pending", "queued")),
        "avg_reality_score": round(stat_mean([parse_float(w.get("reality_score")) for w in scored]), 1) if scored else 0.0,
        "calibration_enabled": calibration_enabled(),
    }


@app.get("/backtests/calibration", response_class=HTMLResponse)
def backtests_calibration_page(request: Request) -> HTMLResponse:
    return templates.TemplateResponse(
        request,
        "backtests_calibration.html",
        page_context_bundle("dashboard", "Backtest Calibration", page_title="Backtest Calibration Lab",
                            **backtest_calibration_context()),
    )


@app.post("/api/backtests/calibration/run")
async def run_calibration_now(request: Request) -> RedirectResponse:
    try:
        run_backtest_calibration_cycle()
    except Exception as exc:  # noqa: BLE001
        log_maintenance("backtesting", "warning", f"Manual calibration cycle failed: {exc}")
    return RedirectResponse(url="/backtests/calibration", status_code=303)
...
def universe_backtest_run_page(request: Request, run_id: str) -> HTMLResponse:
    ...
    return templates.TemplateResponse(
        ...
        page_context_bundle(
            ...
            record=record,
            tape=tape,
            inspection=backtest_inspection(record),
        ),
    )
...
def versions_page(request: Request) -> HTMLResponse:
    ctx = versions_context()
    ctx["environments"] = environments_context()
    return templates.TemplateResponse(
        request,
        "versions.html",
        page_context_bundle("dashboard", "Versions", page_title="Versions", **ctx),
    )


@app.get("/versions/compare", response_class=HTMLResponse)
def versions_compare_page(request: Request) -> HTMLResponse:
    team_id = request.query_params.get("team", "").strip()
    try:
        a_id = int(request.query_params.get("a", "0"))
        b_id = int(request.query_params.get("b", "0"))
    except ValueError:
        a_id = b_id = 0
    version_a = strategy_version_by_id(a_id) if a_id else None
    version_b = strategy_version_by_id(b_id) if b_id else None
    if version_a:
        version_a["stats"] = version_stats(str(version_a["strategy_id"]), int(version_a["id"]))
    if version_b:
        version_b["stats"] = version_stats(str(version_b["strategy_id"]), int(version_b["id"]))
    instance = next((i for i in list_instances() if str(i.get("id")) == team_id), {})
    return templates.TemplateResponse(
        request,
        "versions_compare.html",
        page_context_bundle(
            "dashboard", "Compare Versions", page_title="Compare Versions",
            team_id=team_id,
            display_name=instance.get("display_name") or team_id,
...
def _version_form_value(payload: dict[str, list[str]], key: str) -> str:
    return payload.get(key, [""])[0].strip()


@app.post("/versions/create")
async def versions_create_route(request: Request) -> RedirectResponse:
    payload = urllib.parse.parse_qs((await request.body()).decode("utf-8"))
    strategy_id = _version_form_value(payload, "strategy_id")
    change_type = _version_form_value(payload, "change_type") or "other"
    change_summary = _version_form_value(payload, "change_summary")
    reason = _version_form_value(payload, "reason")
    verdict = _version_form_value(payload, "verdict")
    deploy_now = bool(_version_form_value(payload, "deploy_now"))
    evidence_text = _version_form_value(payload, "evidence")
    # Governance: a major version must carry a summary, a reason, and evidence.
    if not (strategy_id and change_summary and reason and evidence_text):
        return RedirectResponse(url="/versions?error=missing_required", status_code=303)
    version = create_strategy_version(
        strategy_id, change_type, change_summary, reason,
        evidence={"notes": evidence_text}, verdict=verdict, created_by="human",
    )
    if deploy_now and version.get("id"):
        deploy_strategy_version(int(version["id"]))
    return RedirectResponse(url=f"/versions#{strategy_id}", status_code=303)


@app.post("/versions/{version_id}/deploy")
async def versions_deploy_route(request: Request, version_id: int) -> RedirectResponse:
    version = strategy_version_by_id(version_id)
    if not version:
        return RedirectResponse(url="/versions?error=not_found", status_code=303)
    # Governance re-check at the boundary: never deploy without reason + summary + evidence.
    evidence = version.get("evidence_snapshot") or {}
    if not (str(version.get("reason_for_change") or "").strip() and str(version.get("change_summary") or "").strip() and evidence):
        return RedirectResponse(url="/versions?error=missing_required", status_code=303)
    deploy_strategy_version(version_id)
    return RedirectResponse(url=f"/versions#{version['strategy_id']}", status_code=303)


@app.post("/versions/{version_id}/retire")
async def versions_retire_route(request: Request, version_id: int) -> RedirectResponse:
    version = strategy_version_by_id(version_id)
    retire_strategy_version(version_id)
    anchor = f"#{version['strategy_id']}" if version else ""
    return RedirectResponse(url=f"/versions{anchor}", status_code=303)


@app.post("/versions/{version_id}/note")
async def versions_note_route(request: Request, version_id: int) -> RedirectResponse:
    payload = urllib.parse.parse_qs((await request.body()).decode("utf-8"))
    note = _version_form_value(payload, "note")
    version = strategy_version_by_id(version_id)
    if note:
        add_version_note(version_id, note)
    anchor = f"#{version['strategy_id']}" if version else ""
    return RedirectResponse(url=f"/versions{anchor}", status_code=303)


@app.post("/versions/{version_id}/mark-pathological")
async def versions_mark_pathological_route(request: Request, version_id: int) -> RedirectResponse:
    payload = urllib.parse.parse_qs((await request.body()).decode("utf-8"))
    verdict = _version_form_value(payload, "verdict")
    evidence_text = _version_form_value(payload, "evidence")
    version = strategy_version_by_id(version_id)
    mark_version_pathological(version_id, evidence={"notes": evidence_text} if evidence_text else None, verdict=verdict)
    anchor = f"#{version['strategy_id']}" if version else ""
    return RedirectResponse(url=f"/versions{anchor}", status_code=303)


# --- Competition Environment migration GATE -------------------------------
# Parked at the gate: these endpoints PERFORM the universe-standardization
# migration (rewrite live configs to ATL RemotePairList + deploy Environment v2).
# They run ONLY on an explicit operator POST — nothing fires automatically.

@app.post("/environments/migrate-all")
async def environments_migrate_all_route(request: Request) -> RedirectResponse:
    payload = urllib.parse.parse_qs((await request.body()).decode("utf-8"))
    if not _version_form_value(payload, "confirm"):
        return RedirectResponse(url="/versions?error=confirm_required#environments", status_code=303)
    migrate_major_league_to_atl_remote(fire=True)
    return RedirectResponse(url="/versions#environments", status_code=303)


@app.post("/environments/{team_id}/migrate")
async def environments_migrate_one_route(request: Request, team_id: str) -> RedirectResponse:
    payload = urllib.parse.parse_qs((await request.body()).decode("utf-8"))
    if not _version_form_value(payload, "confirm"):
        return RedirectResponse(url="/versions?error=confirm_required#environments", status_code=303)
    migrate_runtime_to_atl_remote(team_id, fire=True)
    return RedirectResponse(url=f"/versions#{team_id}", status_code=303)
...
def ai_settings_page(request: Request) -> HTMLResponse:
    return templates.TemplateResponse(
        request,
        "ai_settings.html",
        page_context_bundle("general", "AI Settings", page_title="AI Settings",
                            settings=app_settings_snapshot(), llm_health=llm_health()),
    )
