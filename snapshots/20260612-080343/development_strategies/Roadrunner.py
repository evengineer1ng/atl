...
class Roadrunner(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: pd.DataFrame, metadata: dict) -> pd.DataFrame:
        ...
        # --- ENTRY GENOME ---
        # Confluence scoring replaces the previous hard-Boolean gate.
        # Five independent axes sum to a maximum of 100 points.
        # A directional score >= 60 qualifies for entry after extreme disqualifiers.
        ...
        mom_strong_l = (dataframe['adx'] > 28) & (dataframe['plus_di'] > dataframe['minus_di']) & (dataframe['adx_slope'] > 0)
        mom_med_l = (dataframe['adx'] > 22) & (dataframe['plus_di'] > dataframe['minus_di'])
        ...
        mom_strong_s = (dataframe['adx'] > 28) & (dataframe['minus_di'] > dataframe['plus_di']) & (dataframe['adx_slope'] > 0)
        mom_med_s = (dataframe['adx'] > 22) & (dataframe['minus_di'] > dataframe['plus_di'])
        ...
        # AXIS 3: BREAKOUT ESCALATION (max 25)
        break_dc15_atr_l = dataframe['close'] > (dataframe['dc_high_15'] + 0.15 * dataframe['atr_14'])
        ...
        break_dc15_atr_s = dataframe['close'] < (dataframe['dc_low_15'] - 0.15 * dataframe['atr_14'])
        ...
        # DISQUALIFIERS: only extremes or missing data are barred
        disqual_long = (dataframe['rsi'] > 88) | dataframe['atr_14'].isna()
        disqual_short = (dataframe['rsi'] < 12) | dataframe['atr_14'].isna()

        dataframe.loc[(score_long >= 60) & (~disqual_long), 'enter_long'] = 1
        dataframe.loc[(score_short >= 60) & (~disqual_short), 'enter_short'] = 1

        # Expose scores for diagnostics and downstream seam-tracing
    ...
    def custom_exit(self, pair: str, trade, current_time: datetime, current_rate: float,
        ...
        if not trade.is_short:
            if (last_candle['close'] < last_candle['ema_21']) and (last_candle['adx'] < 22) and (last_candle['plus_di'] < last_candle['minus_di']):
                return 'mom_death_long'
            if (last_candle['rsi'] > 82) and last_candle['bearish'] and (current_profit > 0.02):
                return 'blowoff_long'
        else:
            if (last_candle['close'] > last_candle['ema_21']) and (last_candle['adx'] < 22) and (last_candle['minus_di'] < last_candle['plus_di']):
    ...
    def adjust_trade_position(self, trade, current_time: datetime, current_rate: float,
        ...
        if not trade.is_short:
            if (last_candle['trend_up'] and last_candle['adx'] > 28 and
                last_candle['plus_di'] > last_candle['minus_di'] and
                last_candle['close'] > last_candle['ema_8']):
                return trade.stake_amount * 0.35
        else:
            if (last_candle['trend_down'] and last_candle['adx'] > 28 and
